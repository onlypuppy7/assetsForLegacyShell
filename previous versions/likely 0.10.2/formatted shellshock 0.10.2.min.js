...
function onCanvasMouseMove(e) {
    if (document.pointerLockElement && me && me.hp > 0) {
        ...
        var r = e.movementY;
        lastMouseMovement.y && Math.sign(r) != Math.sign(lastMouseMovement.y) && (r = 0), lastMouseMovement.y = r, me.viewYaw = Math.radAdd(me.viewYaw, t * i), me.pitch = Math.max(Math.min(me.pitch + r * settings.mouseInvert * i, 1.5), -1.5)
    }
}

function addExplosionSprite(e, i, t, r, n, a, o, d, s) {
    var y = new BABYLON.Sprite("", e);
    y.animSpeed = i, y.position.x = t, y.position.y = r, y.position.z = n, y.baseSize = s, y.dx = a, y.dy = o, y.dz = d, y.df = 0, y.anim = 0, y.color.r = smokeColor.r, y.color.g = smokeColor.g, y.color.b = smokeColor.b, y.color.a = smokeColor.a
}

function addExplosion(e, i, t, r) {
    for (var n = 0; n < Math.floor(r / 2); n++) {
        var a = .04 * Math.random() + .02,
            ...
            l = (2 * Math.random() - 1) * d;
        addExplosionSprite(explosionSmokeManager, a, e, i, t, s, y, l, o), addExplosionSprite(explosionFireManager, a, e, i, t, s, y, l, o)
    }
    if (me) {
        var x = Math.length3(me.x - e, me.y - i, me.z - t);
        x < 5 && (shake = Math.min(7, shake + Math.clamp(r - 25 * x, 0, 5)))
    }
}
...
function onLoadingComplete() {
    if (scope = new Scope, hitIndicator = new HitIndicator, reticle = new Reticle, Meshes.muzzleFlash.material = Materials.muzzleFlash, Meshes.bullet.material = Materials.bullet, Meshes.egg.material = Materials.eggShell, Meshes.eggWhite.material = Materials.eggWhite, Meshes.eggYolk.material = Materials.eggYolk, Meshes.grenade.material = Materials.emissive, Meshes.ammo.material = Materials.standardInstanced, Meshes.grenadeItem.material = Materials.standardInstanced, munitionsManager = new MunitionsManager, itemManager = new ItemManager, buildMapMesh(), bulletHoleManager = new BABYLON.SpriteManager("", "img/bulletHoles.png?v=1", 200, 32, scene), bulletHoleManager.fogEnabled = !0, bulletHoleManager.addHole = function (e, i, t, r) {
        ...
        setUpSocket();
        var e = new Comm.output(1);
        e.packInt8(Comm.clientReady), ws.send(e.buffer)
    }
}
    ...
    ws.onmessage = function (e) {
        for (var i, t = new Comm.input(e.data); i = t.unPackInt8U();) switch (i) {
            ...
            case Comm.addPlayer:
                ...
                r.name = fixNameWidth(r.name), players[r.id] || (meId == r.id && addChat("Welcome, " + r.name + "!"), addPlayer(r));
                break;
            case Comm.removePlayer:
                removePlayer(y = t.unPackInt8U());
                break;
            case Comm.spawnItem:
                var n = t.unPackInt16U(),
                    a = t.unPackInt8U(),
                    o = t.unPackFloat(),
                    d = t.unPackFloat(),
                    s = t.unPackFloat();
                itemManager.spawnItem(n, a, o, d, s);
                break;
            case Comm.collectItem:
                var y = t.unPackInt8U(),
                    a = t.unPackInt8U(),
                    n = t.unPackInt16U();
                itemManager.collectItem(a, n), y == meId && (players[y].collectItem(a), updateAmmoUi());
                break;
            case Comm.keyDown:
                var y = t.unPackInt8U(),
                    l = t.unPackInt8U();
                (W = players[y]) && (W.controlKeys |= l);
                break;
            case Comm.keyUp:
                var y = t.unPackInt8U(),
                    l = t.unPackInt8U();
            ...
            case Comm.die:
                ...
                if (Sounds.death[p].setPosition(m), Sounds.death[p].play(), x != meId) {
                    if (z == meId) {
                        ...
                        var w = 1.5,
                            M = setInterval(function () {
                                g.style.transform = "scale(" + w + "," + w + ")", (w -= .025) <= 1 && clearInterval(M)
                            }, 15);
                    ...
                    var w = 2,
                        M = setInterval(function () {
                            v.style.transform = "scale(" + w + "," + w + ")", (w -= .025) <= 1 && clearInterval(M)
                        }, 15),
                        B = document.getElementById("RESPAWN");
                    B.innerText = "";
                    var S = setInterval(function () {
                        B.innerText = "You may respawn in " + respawnTime, --respawnTime < 0 && (clearInterval(S), B.innerText = "CLICK TO RESPAWN!")
                    }, 1e3)
                }
                ...
                if (n == meId) {
                    ...
                    for (var E = b, P = 1e6, L = b; L != W.stateIdx;) {
                        var O = W.previousStates[L],
                            N = Math.sqrt(Math.pow(o - O.x, 2) + Math.pow(d - O.y, 2) + Math.pow(s - O.z, 2));
                        N < P && (E = L, P = N), L = (L + 1) % 256
                    }
                    var L = E,
                        F = (W.previousStates[L], o - W.previousStates[L].x),
                        T = d - W.previousStates[L].y,
                        Y = s - W.previousStates[L].z;
                    W.x += F;
                    for (L = 0; W.collidesWithMap() && (W.x -= F, F *= .5, W.x += F, !(++L > 100)););
                    for (W.z += Y, L = 0; W.collidesWithMap() && (W.z -= Y, Y *= .5, W.z += Y, !(++L > 100)););
                    T >= .9 && (W.y = d)
                } else W.x = o, W.y = d, W.z = s, W.viewYaw = k, W.moveYaw = k, W.pitch = I;
                break;
            case Comm.fireBullet:
                var n = t.unPackInt8U(),
                    ...
                    R = t.unPackFloat();
                if (!(W = players[n])) break;
                n != meId && (W.actor.head.rotation.x = W.pitch, W.actor.mesh.rotation.y = W.viewYaw, W.actor.fire(), W.weapon.actor.fire()), munitionsManager.fireBullet(W, {
                    x: o,
                    y: d,
                    z: s
                }, {
            ...
            case Comm.fireShot:
                var n = t.unPackInt8U(),
                    ...
                    U = t.unPackInt8U(),
                    W = players[n];
                n != meId && (W.actor.head.rotation.x = W.pitch, W.actor.mesh.rotation.y = W.viewYaw, W.actor.fire(), W.weapon.actor.fire()), Math.seed = U;
                for (L = 0; L < 20; L++) {
                    var H = Math.normalize3({
                        x: G + Math.seededRandom(-.15, .15),
                        y: D + Math.seededRandom(-.1, .1),
                    ...
                    munitionsManager.fireBullet(W, {
                        ...
                        y: d,
                        z: s
                    }, H, W.weapon.damage, W.weapon.ttl, W.weapon.velocity * Math.seededRandom(.9, 1.1))
                }
                break;
            case Comm.throwGrenade:
                var n = t.unPackInt8U(),
                    ...
                    Y = t.unPackFloat();
                if (!(W = players[n])) break;
                W.grenadeCount--, updateAmmoUi(), n != meId && (W.actor.head.rotation.x = W.pitch, W.actor.mesh.rotation.y = W.viewYaw), W.actor.throwGrenade(), munitionsManager.throwGrenade(W, {
                    x: o,
                    y: d,
                    z: s
                }, {
            ...
            case Comm.stowWeapon:
                n = t.unPackInt8U();
                (W = players[n]) && W.weapon.actor.stow();
                break;
            case Comm.equipWeapon:
                var n = t.unPackInt8U(),
                    q = t.unPackInt8U();
                if (!(W = players[n])) break;
                players[n].equipWeapon(q);
                break;
            case Comm.hitMe:
                var K = t.unPackInt8U(),
                    F = t.unPackFloat(),
                    Y = t.unPackFloat();
                me.hp = K, me.actor.hit(), hitIndicator.hit(F, Y);
                break;
            case Comm.hitThem:
                var n = t.unPackInt8U(),
                    K = t.unPackInt8U();
                (W = players[n]).hp = K, W.actor.hit(), K > 0 && shellFragBurst(W.actor.mesh, 100, 1);
                break;
            case Comm.respawn:
                var n = t.unPackInt8U(),
                    o = t.unPackFloat(),
                    d = t.unPackFloat(),
                    s = t.unPackFloat();
                (W = players[n]).respawn(o, d, s), n == meId && (document.getElementById("DEATH_BOX").style.display = "none", camera.position = BABYLON.Vector3.Zero(), camera.rotation = BABYLON.Vector3.Zero(), camera.rotationQuaternion = BABYLON.Quaternion.Zero(), camera.parent = me.actor.eye, camera.lockedTarget = null)
        }
    }
}
...
function createMapCells() {
            ...
            if (i > 0) {
                ...
                try {
                    SPS.addShape(Meshes.map[i][e], r.length, {
                        positionFunction: function (t, a, o) {
                            t.position.x = r[n].x + .5, t.position.y = r[n].y, t.position.z = r[n].z + .5, t.rotation.x = .5 * -Math.PI, t.rotation.y = r[n].dir * rotInc, t.alive = !1, map.data[r[n].x][r[n].y][r[n].z] = {
                                cat: i,
                                dec: e,
                                dir: r[n].dir
                            }, n++
                    ...
                    alert("Cache error. Clear your hosted app data (Chrome) and reload the page!")
                }
            }
        })
    });
    ...
    SPS.addShape(Meshes.mountains, 3, {
        positionFunction: function (e, i, t) {
            e.position.x = map.width / 2, e.position.y = 0, e.position.z = map.depth / 2, e.scaling.x = n[t].w, e.scaling.y = n[t].h, e.scaling.z = n[t].w, e.rotation.y = n[t].r, e.alive = !1
        }
...
function update() {
    ...
    fpsTotal += i, ++fpsSamples % 10 == 0 && (e.innerHTML = "FPS: " + i);
    var t = Date.now() - lastTimeStamp;
    lastTimeStamp += t, t /= 17, Object.keys(players).forEach(function (e) {
        var i = players[e];
        i && (i.update(t), i.actor && i.actor.update(t))
    ...
    }, hitIndicator.update(t), reticle.update(t), me.weapon && 0 == me.weapon.ammo.rounds && (me.stateIdx % 20 == 0 ? document.getElementById("AMMO").style.color = "#f00" : me.stateIdx % 20 == 10 && (document.getElementById("AMMO").style.color = "#fff")), me.grenadeCountdown > 0 && grenadeThrowPower > -1e3 && (grenadeThrowPower = Math.min(grenadeThrowPower + .015, 1)) > 0 && (document.getElementById("grenadeThrowContainer").style.visibility = "visible", document.getElementById("grenadeThrow").style.height = 100 * grenadeThrowPower + "%")), munitionsManager.update(t), itemManager.update(t), updateExplosions(explosionSmokeManager, t), updateExplosions(explosionFireManager, t)
}
...
function addPlayer(e) {
    var i = new Player(e);
    i.id == meId ? ((me = i).ws = ws, camera.parent = me.actor.eye) : (i.equipWeapon(i.weaponIdx), i.isDead() && i.actor.die()), players[e.id] = i, rebuildPlayerList()
}

function removePlayer(e) {
    players[e].actor.remove(), delete players[e], rebuildPlayerList()
}

function reloadWeapon() {
    if (me.weapon.startReload()) {
        var e = new Comm.output(1);
        e.packInt8(Comm.startReload), ws.send(e.buffer)
    }
}

function rebuildPlayerList() {
    var e = Object.keys(players).sort(function (e, i) {
            return players[i].killStreak - players[e].killStreak
        }),
        i = document.getElementById("playerList").children,
        t = 0;
    for (e.forEach(function (e) {
            i[t].style.display = "block", i[t].children[0].innerText = players[e].name, i[t].children[1].innerText = players[e].killStreak, i[t].className = e == meId ? "thisPlayer" : "otherPlayer", t++
        }); t < 20;) i[t].style.display = "none", t++
}
...
function updateAmmoUi() {
    var e = players[meId],
        i = document.getElementById("WEAPON_NAME");
    i.innerHTML = e.weapon.name, (i = document.getElementById("AMMO")).style.color = "#fff", i.innerHTML = e.weapon.ammo.rounds + "/" + e.weapon.ammo.store;
    ...
    if (!me.isDead()) {
        ...
        switch (i) {
            ...
            case "jump":
                ...
                r.packInt8(Comm.jump), ws.send(r.buffer), me.jump(), me.previousStates[me.stateIdx].jump = !0, me.previousStates[me.stateIdx].dy = me.dy;
                break;
            case "fire":
                document.pointerLockElement && me && (me.grenadeCountdown > 0 ? me.actor.cancelGrenade() : (me.previousStates[me.stateIdx].fire = !0, me.weapon && (me.weapon.triggerPulled = !0), me.queueShot(), 0 == me.weapon.ammo.rounds && reloadWeapon()));
                break;
            case "grenade":
                document.pointerLockElement && me && (grenadeThrowPower = -.15, me.actor.reachForGrenade());
                break;
            case "scope":
    ...
    if (!me.isDead()) {
        ...
        switch (i) {
            ...
            case "scope":
                ...
                break;
            case "grenade":
                document.pointerLockElement && me && (document.getElementById("grenadeThrowContainer").style.visibility = "hidden", me.throwGrenade(grenadeThrowPower));
                break;
            case "up":
            case "down":
            case "left":
...
function onKeyDown(e) {
    if (!keyIsDown[e.keyCode]) {
        ...
        "" != inputTally && "" == (inputTally = inputTally.replace(i, "")) && (document.getElementById("help").style.display = "none", localStorage.setItem("hideHelp", 1)), " " == i && (i = "SPACE", e.preventDefault()), inputDown(i)
    }
}
...
function inviteFriends() {
    ...
    document.getElementById("friendCode").innerText = "shellshock.io/#" + e, document.getElementById("inviteFriends").style.display = "block"
}
...
function isMeshVisible(e, i) {
    for (var t = i || e.getBoundingInfo().boundingBox.center.z, r = e.position.x - camera.globalPosition.x, n = e.position.y + t - camera.globalPosition.y, a = e.position.z - camera.globalPosition.z, o = Math.length3(r, n, a), d = Math.normalize3({
            x: r,
            y: n,
            z: a
        }, .9), s = camera.globalPosition.x, y = camera.globalPosition.y, l = camera.globalPosition.z, x = 0, z = 0; z < o - .9; z += .9) {
        var h = collidesWithCell(s += d.x, y += d.y, l += d.z);
        if (h && (h.cel.cat == MAP.ground || h.cel.cat == MAP.block) && 2 == ++x) return void(e.isVisible && e.setVisible(!1))
    }
    e.isVisible || e.setVisible(!0)
}

function GrenadeActor(e) {
    this.grenade = e, this.mesh = Meshes.grenade.clone(""), this.mesh.setEnabled(!1), this.flashColor = new BABYLON.Color3(1, 0, 0), this.explodeSound = Sounds.grenade.explode.clone(), this.pinSound = Sounds.grenade.pin.clone(), this.beepSound = Sounds.grenade.beep.clone(), this.mesh.attachSound(this.pinSound), this.mesh.attachSound(this.beepSound), this.beep = !1
}
...
function onResourcesLoaded() {
    function e() {
        ...
        for (z in l) {
            var e = (d[selectedClass].weapon[z] + l[z].min) / (l[z].max - Math.abs(l[z].min));
            e = Math.floor(95 * e) + 5, document.getElementById(l[z].name).style.width = e + "%"
        }
    }
    ...
    meId = 0;
    var a = new BABYLON.StandardMaterial("groundMat", scene);
    a.diffuseColor = new BABYLON.Color3(.4, .3, .2);
    var o = BABYLON.Mesh.CreatePlane("ground", 100, scene);
    o.position.y = -.32, o.rotation.x = Math.PI / 2, o.receiveShadows = !0, o.material = a, light.position.y = 2;
    for (var d = [], s = 0; s < n; s++) {
        var y = new function (e, i, t, r) {
            this.id = 1, this.name = "", this.x = e, this.y = i, this.z = t, this.hp = 100, this.actor = new PlayerActor(this), this.actor.mesh.setPivotPoint(new BABYLON.Vector3(0, 0, -1.6), BABYLON.Space.GLOBAL), this.actor.head.rotation.y = 2.5, this.weapon = new classes[r].weapon(this), this.weapon.equip()
        }(0, -.32, 1.6, s);
        y.actor.mesh.rotation.y = s * Math.PI / (n / 2), d.push(y)
    }
    for (var l = {
            totalDamage: {
                name: "damage",
        ...
        var x = d[s].weapon;
        for (var z in l) l[z].flip ? (l[z].max = Math.max(l[z].max, -x[z]), l[z].min = Math.min(l[z].min, -x[z])) : (l[z].max = Math.max(l[z].max, x[z]), l[z].min = 0)
    }
    var h = -(selectedClass = getStoredNumber("selectedClass", Math.randomInt(0, classes.length)));
    window.selectClass = function (i) {
        selectedClass = Math.mod(selectedClass + i, n), h -= i, e()
    }, e(), BABYLON.Engine.audioEngine.setGlobalVolume(settings.volume), engine.runRenderLoop(function () {
        scene.render();
        var e = h / 5;
        h -= e;
        for (var i = 0; i < n; i++) d[i].actor.mesh.rotation.y += e * (Math.PI / (n / 2))
    })
}
...
function login(e) {
    ...
    var i, t = selectedServer,
        ...
        n = "ws://";
    "https:" === location.protocol && (n = "wss://");
    var a = document.getElementById("joinCode").value.trim();
    if (e) r = Comm.createPrivateGame, i = n + servers[t].address;
    else if ("" != a) {
        r = Comm.joinPrivateGame, a.startsWith("#") && (a = a.substr(1)), t = Number.parseInt(a.substr(0, 1), 36), uniqueId = Number.parseInt(a.substr(1, 3), 36), uniqueKey = Number.parseInt(a.substr(4, 2), 36);
        try {
            i = n + servers[t].address
        } catch (e) {
        ...
        selectedServer = t
    } else i = n + servers[t].address;
    (ws = new WebSocket(i)).binaryType = "arraybuffer", 0 != (username = document.getElementById("username").value.trim()).length && (document.getElementById("alert").focus(), openAlertDialog("CONNECTING", "Please wait!", null, null, !0), ws.onopen = function (e) {
        ...
        var i = 7 + 2 * username.length;
        facebookId && (i += 2 * facebookId.length + 6);
        var t = new Comm.output(i);
        t.packInt8(Comm.login), t.packInt8(r), t.packInt16(uniqueId), t.packInt16(uniqueKey), t.packInt8(selectedClass), t.packString(username), facebookId && (t.packInt32(gameSession), t.packString(facebookId)), ws.send(t.buffer)
    }, ws.onclose = function (e) {
        e.code == CloseCode.gameNotFound ? (openAlertDialog("GAME NOT FOUND", "Sorry! This game ID is either<br>invalid, or no longer exists.", {
            label: "OK"
        ...
        switch (i.unPackInt8U()) {
            case Comm.loggedIn:
                meId = i.unPackInt8U(), uniqueId = i.unPackInt16U(), uniqueKey = i.unPackInt16U(), mapIdx = i.unPackInt8U(), privateGame && inviteFriends(), inGame = !0, startGame()
        }
    })
}
...
function setDetailSettingsVisibility(e) {
    ...
    scene && (document.getElementById("enableShadows").checked = scene.shadowsEnabled, document.getElementById("highRes").checked = 1 == engine.getHardwareScalingLevel())
}
...
function getStoredNumber(e, i) {
    var t = Number(localStorage.getItem(e));
    return t || i
}
...
function openAlertDialog(e, i, t, r, n) {
    ...
    t ? (a.style.display = "inline-block", a.innerHTML = t.label || "OK", a.style.width = t.width || "80px", a.onclick = t.onclick || closeAlertDialog) : a.style.display = "none";
    var a = document.getElementById("alertButton2");
    r ? (a.style.display = "inline-block", a.innerHTML = r.label || "Cancel", a.style.width = r.width || "80px", a.onclick = r.onclick || closeAlertDialog) : a.style.display = "none", n ? startAlertBar() : (document.getElementById("alertFooter").style.display = "none", clearInterval(alertBarInterval)), document.exitPointerLock(), window.onkeydown = null, window.onkeyup = null
}
...
function showMainMenu() {
    if (openAlertDialog("LOADING", "Just a moment!", null, null, !0), document.getElementById("homeButton").style.visibility = "hidden", document.getElementById("friendsButton").style.visibility = "hidden", document.getElementById("game").style.display = "none", document.getElementById("inviteFriends").style.display = "none", resetGameUI(), inGame && (canvas.removeEventListener("click", onCanvasClick), canvas.removeEventListener("mousedown", onCanvasMouseDown), canvas.removeEventListener("mouseup", onCanvasMouseUp), canvas.removeEventListener("mousemove", onCanvasMouseMove), ws.close(CloseCode.mainMenu), ws = null, releaseKeys(), gameStartTime > 0)) {
        ...
        ga("send", "timing", "game", "play time", e), fbq("trackCustom", "EndGame", {
            timePlayed: e
        })
    }
...
function fixNameWidth(e) {
    ...
    for (i.font = "1em Nunito, sans-serif";;) {
        if (i.measureText(e).width < 100) break;
...
function PlayerActor(e) {
    if (this.player = e, this.mesh = new BABYLON.AbstractMesh("player", scene), this.bodyMesh = Meshes.egg.clone("egg", this.mesh), this.bodyMesh.position.y = .32, this.bodyMesh.parent = this.mesh, this.bodyMesh.player = this.player, this.bodyMesh.material = Materials.eggShell, this.player.id == meId && this.bodyMesh.setEnabled(!1), this.explodeMesh = Meshes.eggExplode.clone("explode", this.mesh), this.explodeMesh.position.y = .32, this.explodeMesh.parent = this.mesh, this.explodeMesh.setMaterial(Materials.normalBackface), this.explodeMesh.setEnabled(!1), this.whiteMesh = Meshes.eggWhite.clone("white", this.mesh), this.whiteMesh.parent = this.explodeMesh, this.whiteMesh.setEnabled(!1), this.yolkMesh = Meshes.eggYolk.clone("yolk", this.mesh), this.yolkMesh.parent = this.explodeMesh, this.yolkMesh.setEnabled(!1), this.head = new BABYLON.AbstractMesh("head", scene), this.head.parent = this.mesh, this.head.position.y = .3, this.head.position.z = 0, this.gunContainer = new BABYLON.AbstractMesh("gunContainer", scene), this.gunContainer.parent = this.head, this.gunContainer.rotation.y = -.14, this.gunContainer.rotation.x = -.035, this.eye = new BABYLON.AbstractMesh("eye", scene), this.eye.position.y = .1, this.eye.position.x = .1, this.eye.parent = this.head, this.gripHand = Meshes.gripHand.clone("gripHand", this.gunContainer), this.gripHand.parent = this.gunContainer, this.foreHand = Meshes.foreHand.clone("foreHand", this.gunContainer), this.foreHand.parent = this.gunContainer, shake = 0, shadowGen && shadowGen.getShadowMap().renderList.push(this.bodyMesh), inGame)
        if (this.player.id == meId) this.gripHand.setRenderingGroupId(1), this.foreHand.setRenderingGroupId(1);
            ...
            var i = new BABYLON.DynamicTexture("nameTexture", 256, scene, !1, 1);
            this.nameSurface.material.diffuseTexture = i, this.nameSurface.material.diffuseTexture.hasAlpha = !0, this.nameSurface.material.useAlphaFromDiffuseTexture = !0, this.nameSurface.material.backFaceCulling = !1, i.drawText(this.player.name, 9 * (14 - this.player.name.length), 30, "bold 30px Nunito", "white", "transparent"), this.nameSurface.parent = this.mesh, this.nameSurface.billboardMode = BABYLON.AbstractMesh.BILLBOARDMODE_ALL, this.nameSurface.position.y = .475
...
function loadMaterials() {
    ...
    e.push("#define DIRT"), Materials.map = new BABYLON.ShaderMaterial("", scene, "standard", {
        ...
        i.setFloat4("vFogInfos", scene.fogMode, scene.fogStart, scene.fogEnd, scene.fogDensity), i.setColor3("vFogColor", scene.fogColor), i.setFloat("hp", e.player.hp / 100)
        ...
        i.setFloat4("vFogInfos", scene.fogMode, scene.fogStart, scene.fogEnd, scene.fogDensity), i.setColor3("vFogColor", scene.fogColor), i.setColor3("emissiveColor", e.material.emissiveColor || BABYLON.Color3.Black())
    ...
    function n() {
        BABYLON.SceneLoader.ImportMesh("", "models/", e[d] + ".babylon", scene, function (e, s, y) {
            for (var l = 0; l < e.length; l++) {
                var x = e[l];
                x.setEnabled(!1), x.setMaterial(Materials.standard), x.isPickable = !1, t ? i.push(x) : i[x.name] = x
            }
            d++, 0 == --a ? r && r.call(o) : n()
        })
    }
    ...
    if (o.cat) {
        if (o.cat == MAP.ramp) switch (o.dir) {
            case 0:
                ...
                break;
            case 2:
                if (i - n > 1 - (t - a)) return !1;
                break;
            case 1:
                if (i - n > e - r) return !1;
                break;
            case 3:
                if (i - n > 1 - (e - r)) return !1
...
function Gun(e) {
    this.player = e, this.rofCountdown = 0, this.shotsQueued = 0, this.reloadCountdown = 0, this.triggerPulled = !1, this.equipCountdown = 0
}
...
function Player(e) {
    this.id = e.id, this.name = e.name, this.charClass = e.charClass, this.x = e.x, this.y = e.y, this.z = e.z, this.dx = e.dx, this.dy = e.dy, this.dz = e.dz, this.viewYaw = e.viewYaw, this.controlKeys = e.controlKeys, this.moveYaw = e.moveYaw, this.pitch = e.pitch, this.totalKills = e.totalKills, this.totalDeaths = e.totalDeaths, this.killStreak = e.killStreak, this.bestKillStreak = e.bestKillStreak, this.hp = e.hp, this.weaponIdx = e.weaponIdx, this.equipWeaponIdx = 0, this.stowWeaponCountdown = 0, this.shotSpread = 0, this.jumping = !1, this.climbing = !1, this.bobble = 0, this.stateIdx = 0, this.timeOfDeath = 0, this.previousStates = Array(256).fill({
        delta: 0,
        moveYaw: 0,
        fire: !1,
        jump: !1,
        ...
        dz: 0,
        controlKeys: 0
    }), void 0 !== PlayerActor && (this.actor = new PlayerActor(this)), this.weapons = [new Cluck9mm(this), new classes[this.charClass].weapon(this)], this.weapon = this.weapons[0], this.grenadeCount = 1, this.grenadeCapacity = 3, this.grenadeThrown = !1, this.grenadeCountdown = 0
}
...
var bulletHoleManager, explosionSmokeManager, explosionFireManager, respawnTime, players, keyIsDown, map, inputTally, light, camera, uiCamera, me, grenadeThrowPower = -1e3,
    lastTimeStamp, lastDelta, fps, fpsSum, fpsIdx, scope, reticle, hitIndicator, munitionsManager, itemManager, lastMouseMovement = {},
...
Scope.prototype.show = function () {
    ...
    var t = Math.pow(.9, e);
    me && !me.isDead() && (camera.position.x *= t, camera.position.z *= t), this.mesh.updateVerticesData(BABYLON.VertexBuffer.ColorKind, this.colors, !0), document.getElementById("healthBar").style.width = me.hp + "%"
...
GrenadeActor.prototype.throw = function () {
    this.mesh.setEnabled(!0), this.mesh.position.x = this.grenade.x, this.mesh.position.y = this.grenade.y, this.mesh.position.z = this.grenade.z, this.pinSound.play(), this.bounce()
    ...
    this.animate(154, 168, function () {
        e.gunMesh.setEnabled(!1), e.clipMesh.setEnabled(!1), e.gun.player.stowWeaponCountdown = 0
    })
    ...
    this.animate(416, 433, function () {
        e.gunMesh.setEnabled(!1), e.gun.player.stowWeaponCountdown = 0
    })
    ...
    for (var i = 0; i < this.pools.length; i++) this.pools[i].forEachActive(function (i) {
        i.update(e), isMeshVisible(i.mesh)
    })
}, ItemManager.prototype.spawnItem = function (e, i, t, r, n) {
    var a = this.pools[i].retrieve(e);
    a.mesh.setEnabled(!0), a.mesh.position.x = t, a.mesh.position.y = r, a.mesh.position.z = n
...
var fbAppId = "503435033333554",
    ...
    ga;
! function (e, i, t, r, n, a, o) {
    e.GoogleAnalyticsObject = n, e[n] = e[n] || function () {
        (e[n].q = e[n].q || []).push(arguments)
    }, e[n].l = 1 * new Date, a = i.createElement(t), o = i.getElementsByTagName(t)[0], a.async = 1, a.src = "https://www.google-analytics.com/analytics.js", o.parentNode.insertBefore(a, o)
}(window, document, "script", 0, "ga"), ga("create", "UA-105800112-1", "auto"), ga("send", "pageview"), window.onmessage = function (e) {
    var i = e.data;
    "function" == typeof window[i.func] && window[i.func].call(null, i.message)
    ...
    function e(e) {
        console.log(e), facebookId = null, gameSession = null, z = e;
    ...
    if (resize(), BABYLON.Engine.isSupported()) {
        for (var i = [{
                ...
                link: "http://store.steampowered.com/app/562230/Space_Tyrant/"
            }], t = (Math.randomInt(0, i.length), document.getElementById("serverSelect")), r = 0; r < servers.length; r++) {
            var n = servers[r];
            (s = document.createElement("option")).textContent = n.name, t.appendChild(s), n.pingEl = new Image, n.pingEl.idx = r, n.pingEl.onerror = function (e) {
                selectedServer = e.target.idx, t.selectedIndex = selectedServer;
                for (var i = 0; i < servers.length; i++) servers[i].pingEl.onerror = null
            }
        }
        for (r = 0; r < servers.length; r++) servers[r].pingEl.src = location.protocol + "//" + servers[r].address;
        settings.volume = getStoredNumber("volume", 1), settings.mouseSensitivity = Math.max(Math.min(11, getStoredNumber("mouseSensitivity", 5)), 0), settings.mouseInvert = getStoredNumber("mouseInvert", 1), settings.holdToAim = getStoredBool("holdToAim", !0), settings.autoDetail = getStoredBool("autoDetail", !0), settings.enableShadows = getStoredBool("enableShadows", !0), settings.highRes = getStoredBool("highRes", !0), setDetailSettingsVisibility(settings.autoDetail), document.getElementById("volume").value = settings.volume, document.getElementById("mouseSensitivity").value = settings.mouseSensitivity, document.getElementById("mouseInvert").checked = -1 == settings.mouseInvert, document.getElementById("holdToAim").checked = settings.holdToAim, document.getElementById("autoDetail").checked = settings.autoDetail, document.getElementById("enableShadows").checked = settings.enableShadows, document.getElementById("highRes").checked = settings.highRes;
        var a = JSON.parse(localStorage.getItem("controlConfig"));
        for (var o in a) a.hasOwnProperty(o) && (inputToControlMap[o] = a[o]);
        ...
        for (var y = document.getElementById("playerList"), l = document.getElementById("playerSlot"), r = 0; r < 20; r++) {
            var x = l.cloneNode(!0);
            y.appendChild(x)
        }
        ...
        if (window.checkLoginState = function () {
                ...
                e.getElementById(t) || ((r = e.createElement(i)).id = t, r.src = "https://connect.facebook.net/en_US/sdk.js#xfbml=1&version=v2.11&appId=503435033333554", n.parentNode.insertBefore(r, n))
        ...
        c.value = getStoredString("lastUsername", ""), c.addEventListener("keyup", function (e) {
            c.value = fixNameWidth(e.target.value), "Enter" != e.code && 13 != e.keyCode || (c.disabled = !0, login())
        }), playOffline ? (ws = {
            send: function () {}
...
PlayerActor.prototype.update = function (e) {
    ...
    this.player.id == meId && (this.scope && this.player.isAtReady() ? (camera.fov = camera.fov + (this.player.weapon.actor.scopeFov - camera.fov) / 3, this.gunContainer.rotation.y *= .667, this.gunContainer.rotation.x *= .667, this.gunContainer.position.x += (-.15 - this.gunContainer.position.x) / 3, this.gunContainer.position.y += (this.player.weapon.actor.scopeY - this.gunContainer.position.y) / 4, this.gunContainer.position.z += (-.05 - this.gunContainer.position.z) / 3, this.player.weapon.hasScope && !this.zoomed && camera.fov < this.player.weapon.actor.scopeFov + .05 && (scope.show(), this.gunContainer.setEnabled(!1), this.zoomed = !0)) : (camera.fov = camera.fov + (1.25 - camera.fov) / 3, this.gunContainer.rotation.y += (2 * i - .14 - this.gunContainer.rotation.y) / 3, this.gunContainer.rotation.x += (.75 * r - .035 - this.gunContainer.rotation.x) / 3, this.gunContainer.position.x *= .667, this.gunContainer.position.y *= .667, this.gunContainer.position.z *= .667, this.zoomed && camera.fov > this.player.weapon.actor.scopeFov + .05 && (scope.hide(), this.gunContainer.setEnabled(!0), this.zoomed = !1)));
    var n = this.player.x - this.mesh.position.x,
        a = this.player.y - this.mesh.position.y,
        o = this.player.z - this.mesh.position.z;
    Math.length3(n, a, o) < .9 ? (this.mesh.position.x += n / 6, this.mesh.position.y += a / 2, this.mesh.position.z += o / 6) : (this.mesh.position.x = this.player.x, this.mesh.position.y = this.player.y, this.mesh.position.z = this.player.z);
    var d = .9;
    this.player.id != meId && (d = .5);
    var s = Math.radDifference(this.player.viewYaw, this.mesh.rotation.y),
        y = Math.radDifference(this.player.pitch, this.head.rotation.x);
    this.player.addRotationShotSpread(s, y), this.mesh.rotation.y += s * d, this.head.rotation.x += y * d, this.bodyMesh.rotation.x = this.head.rotation.x / 4;
    var l;
    if (l = this.player.jumping ? 0 : Math.length3(this.player.dx, this.player.dy, this.player.dz), this.bobbleIntensity += (l - this.bobbleIntensity) / 10, this.bodyMesh.rotation.z = 5 * i, this.bodyMesh.position.y = 1.5 * t + .32, shake > 0)
        if ((shake *= .9) < .001) shake = 0;
        else {
            var x = Math.random() * shake - .5 * shake,
                z = Math.random() * shake - .5 * shake,
                h = Math.random() * shake - .5 * shake;
            this.eye.rotation.x += (x - this.eye.rotation.x) / 10, this.eye.rotation.y += (z - this.eye.rotation.y) / 10, this.eye.rotation.z += (h - this.eye.rotation.z) / 10
        }
    else this.eye.rotation.x *= .9, this.eye.rotation.y *= .9, this.eye.rotation.z *= .9;
    this.player.id != meId && isMeshVisible(this.mesh, .31), this.hitSoundDelay = Math.max(this.hitSoundDelay - e, 0)
    ...
    this.bodyMesh.setEnabled(!1), this.head.setEnabled(!1), this.eye.setEnabled(!1), this.player.id == meId && (scope.hide(), this.gunContainer.setEnabled(!0), this.zoomed = !1, this.scope = !1, camera.fov = 1.25, this.player.grenadeCountdown = 0, document.getElementById("grenadeThrowContainer").style.visibility = "hidden"), this.nameSurface && this.nameSurface.setEnabled(!1)
    ...
    this.player.isAtReady() && this.player.grenadeCount > 0 && 0 == this.player.grenadeCountdown && (this.player.grenadeCountdown = 1, this.oldHandPos = this.foreHand.position, this.oldHandRot = this.foreHand.rotationQuaternion, this.oldHandParent = this.foreHand.parent, this.foreHand.parent = this.gunContainer, scene.beginAnimation(this.foreHand, 1300, 1315, !1, 1))
}, PlayerActor.prototype.cancelGrenade = function () {
    this.player.grenadeCountdown = 0, this.foreHand.position = this.oldHandPos, this.foreHand.rotationQuaternion = this.oldHandRot, this.foreHand.parent = this.oldHandParent, this.player.id == meId && (document.getElementById("grenadeThrowContainer").style.visibility = "hidden")
}, PlayerActor.prototype.throwGrenade = function () {
    var e = this;
    this.player.id != meId && (this.oldHandPos = this.foreHand.position, this.oldHandRot = this.foreHand.rotationQuaternion, this.oldHandParent = this.foreHand.parent, this.foreHand.parent = this.gunContainer), scene.beginAnimation(this.foreHand, 1315, 1350, !1, 1, function () {
        e.cancelGrenade()
    })
...
}, Bullet.prototype.fire = function (e, i, t, r, n, a) {
    this.player = e, this.x = i.x, this.y = i.y, this.z = i.z, this.dx = t.x * a, this.dy = t.y * a, this.dz = t.z * a, this.ttl = n, this.damage = r, this.active = !0, this.actor && this.actor.fire()
    ...
    if (e) {
        if (this.actor) {
            ...
            if (i && i.cat > 0 && Meshes.map[i.cat][i.dec]) {
                ...
                if (r.hit) {
                    var n = new BABYLON.Vector3(-this.dx, -this.dy, -this.dz).normalize().scale(.005);
                    bulletHoleManager.addHole(0, r.pickedPoint.x + e.x + .5 + n.x, r.pickedPoint.y + e.y + n.y, r.pickedPoint.z + e.z + .5 + n.z)
                }
            }
        }
        return !0
...
var CloseCode = {
    ...
    GameMap = {
            ...
            for (var i = 0; i < e.width; i++)
                for (var t = 0; t < e.height; t++)
                    for (var r = 0; r < e.depth; r++) {
                        var n = e.data[i][t][r];
                        n.cat && (e.min.data[n.cat] || (e.min.data[n.cat] = {}), e.min.data[n.cat][n.dec] || (e.min.data[n.cat][n.dec] = []), e.min.data[n.cat][n.dec].push({
                            x: i,
                            y: t,
                            z: r,
                            dir: n.dir
        ...
        generateMap: function (e, i, t, r) {
            Math.seed = r;
            var n = {};
            n.width = e, n.depth = i, n.height = t, n.seed = r, n.data = Array(n.width);
            for (s = 0; s < n.width; s++) {
                n.data[s] = Array(n.height);
                for (h = 0; h < n.height; h++) n.data[s][h] = Array(n.depth).fill({})
            }
            for (c = 0; c < n.width * n.depth * n.height * .2;)
                for (var a = Math.seededRandomInt(4, 8), o = Math.seededRandomInt(4, 8), d = Math.seededRandomInt(2, c % n.height), s = Math.seededRandomInt(1, n.width - 1 - a), y = Math.seededRandomInt(1, n.depth - 1 - o), l = (Math.seededRandomInt(1, 4), s); l < s + a; l++)
                    for (p = y; p < y + o; p++)
                        for (var x = 0; x < d; x++) {
                            var z = l == s || p == y || l == s + a - 1 || p == y + o - 1;
                            n.data[l][x][p].cat || c++, n.data[l][x][p] = z ? {
                                cat: 1,
                                dec: 4,
            ...
            for (s = 0; s < n.width; s++)
                for (h = 0; h < n.height; h++)
                    for (y = 0; y < n.depth; y++) 0 == n.data[s][h][y].dec && (n.data[s][h][y] = {});
            for (u = 0; u < n.width * n.depth * n.height / 2; u++) {
                var s = Math.seededRandomInt(0, n.width),
                    h = 2 * Math.seededRandomInt(0, Math.floor(n.height / 2)),
                    y = Math.seededRandomInt(0, n.depth);
                1 == n.data[s][h][y].cat && n.data[s][h][y].dec > 0 && 4 == GameMap.numNeighbors6(n, s, h, y) && (n.data[s][h][y] = {})
            }
            for (var c = 0; c < n.width * n.depth * n.height / 60;) {
                var s = Math.seededRandomInt(1, n.width - 1),
                    h = Math.seededRandomInt(0, n.height - 1),
                    y = Math.seededRandomInt(1, n.depth - 1);
                n.data[s][h][y].cat || 0 != h && 1 != n.data[s][h - 1][y].cat || (1 != n.data[s][h][y + 1].cat || n.data[s][h + 1][y + 1].cat || n.data[s][h][y - 1].cat || 0 != h && 1 != n.data[s][h - 1][y - 1].cat ? 1 != n.data[s + 1][h][y].cat || n.data[s + 1][h + 1][y].cat || n.data[s - 1][h][y].cat || 0 != h && 1 != n.data[s - 1][h - 1][y].cat ? 1 != n.data[s][h][y - 1].cat || n.data[s][h + 1][y - 1].cat || n.data[s][h][y + 1].cat || 0 != h && 1 != n.data[s][h - 1][y + 1].cat ? 1 != n.data[s - 1][h][y].cat || n.data[s - 1][h + 1][y].cat || n.data[s + 1][h][y].cat || 0 != h && 1 != n.data[s + 1][h - 1][y].cat || (n.data[s][h][y] = {
                    cat: 2,
                    dec: 0,
                    dir: 3
                }, n.data[s][h + 1][y] = {}, n.data[s + 1][h + 1][y] = {}, c++) : (n.data[s][h][y] = {
                    cat: 2,
                    dec: 0,
                    dir: 2
                }, n.data[s][h + 1][y] = {}, n.data[s][h + 1][y + 1] = {}, c++) : (n.data[s][h][y] = {
                    cat: 2,
                    dec: 0,
                    dir: 1
                }, n.data[s][h + 1][y] = {}, n.data[s - 1][h + 1][y] = {}, c++) : (n.data[s][h][y] = {
                    ...
                    dec: 0,
                    dir: 0
                }, n.data[s][h + 1][y] = {}, n.data[s][h + 1][y - 1] = {}, c++))
            }
            for (u = 0; u < n.width * n.depth * n.height / 10; u++) {
                var s = Math.seededRandomInt(1, n.width - 1),
                    h = Math.seededRandomInt(0, n.height - 1),
                    y = Math.seededRandomInt(1, n.depth - 1);
                !n.data[s][h][y].cat && (0 == h || 1 == n.data[s][h - 1][y].cat) && GameMap.numNeighbors26(n, s, h, y) < 11 && (n.data[s][h][y] = {
                    cat: 4,
                    dec: 0,
                    dir: 0
                })
            }
            for (var u = 0; u < n.width * n.depth * n.height / 10; u++) {
                var s = Math.seededRandomInt(1, n.width - 1),
                    h = Math.seededRandomInt(0, n.height - 1),
                    y = Math.seededRandomInt(1, n.depth - 1);
                if (!n.data[s][h][y].cat && (0 == h || 1 == n.data[s][h - 1][y].cat && 4 == n.data[s][h - 1][y].dec) && GameMap.numNeighbors26(n, s, h, y) < 11) {
                    n.data[s][h][y] = {
                        cat: 1,
                    ...
                    for (var m = 0; m < 20; m++) {
                        ...
                        if (4 == n.data[p][h][p].cat) break;
                        if (!n.data[p][h][p].cat && (0 == h || 1 == n.data[l][h - 1][p].cat && 4 == n.data[l][h - 1][p].dec)) {
                            n.data[l][h][p] = {
                                cat: 4,
                                dec: 0,
                                dir: 0
            ...
            for (s = 0; s < n.width; s++)
                for (y = 0; y < n.depth; y++)
                    for (h = 0; h < n.height - 1; h++) !n.data[s][h][y].cat && this.numNeighbors6(n, s, h, y) >= 4 && !n.data[s][h + 1][y].cat && (n.data[s][h][y] = {
                        cat: 1,
                        dec: this.firstNeighborDec(n, s, h, y),
                        dir: Math.seededRandomInt(0, 4)
                    });
            for (s = 0; s < n.width; s++)
                for (h = 0; h < n.height; h++)
                    for (y = 0; y < n.depth; y++) n.data[s][h][y].cat && 6 == GameMap.numNeighbors6(n, s, h, y) && (n.data[s][h][y].cat = 1, n.data[s][h][y].dec = 0);
            return GameMap.makeMinMap(n), n
        },
        firstNeighborDec: function (e, i, t, r) {
            for (var n = Math.max(1, i - 1); n <= Math.min(e.width - 2, i + 1); n++)
                for (var a = Math.max(0, t - 1); a <= Math.min(e.height - 1, t + 1); a++)
                    for (var o = Math.max(1, r - 1); o <= Math.min(e.depth - 2, r + 1); o++)
                        if ((i != n || t != a || r != o) && 1 == e.data[n][a][o].cat) return e.data[n][a][o].dec;
            return 0
        },
        numNeighbors6: function (e, i, t, r) {
            for (var n = 0, a = Math.max(1, i - 1); a <= Math.min(e.width - 2, i + 1); a++)
                for (var o = Math.max(0, t - 1); o <= Math.min(e.height - 1, t + 1); o++)
                    for (var d = Math.max(1, r - 1); d <= Math.min(e.depth - 2, r + 1); d++) Math.abs(a - i) + Math.abs(o - t) + Math.abs(d - r) == 1 && 1 == e.data[a][o][d].cat && n++;
            return 0 == t && n++, n
        },
        numNeighbors26: function (e, i, t, r) {
            for (var n = 0, a = Math.max(1, i - 1); a <= Math.min(e.width - 2, i + 1); a++)
                for (var o = Math.max(0, t - 1); o <= Math.min(e.height - 1, t + 1); o++)
                    for (var d = Math.max(1, r - 1); d <= Math.min(e.depth - 2, r + 1); d++) i == a && t == o && r == d || 1 == e.data[a][o][d].cat && n++;
            return 0 == t && (n += 9), n
        }
    };
Grenade.prototype.update = function (e) {
    if (this.ttl <= 0)
            ...
            Object.keys(players).forEach(function (e) {
                ...
                if (i) {
                    var r = t.x - i.x,
                        n = t.y - (i.y + .3),
                        a = t.z - i.z,
                        o = Math.length3(r, n, a),
                        d = Math.max(0, t.damage - 25 * o);
                    if (i && !i.isDead() && d > 0) {
                        for (var s = Math.normalize3({
                                x: r,
                                y: n,
                                z: a
                            }, .25), y = i.x, l = i.y + .3, x = i.z, z = !0, h = 0; h < o; h += .25) {
                            if (collidesWithCell(y, l, x)) {
                                z = !1;
                                break
                            }
                            y += s.x, l -= s.y, x += s.z
                        }
                        z && hitPlayer(i, t.player, d, -r, -a)
                    }
                }
            })
        }
        ...
        var o = .5 * (this.dx + r) * e,
            d = .5 * (this.dy + n) * e,
            s = .5 * (this.dz + a) * e;
        this.dx *= Math.pow(.99, e), this.dz *= Math.pow(.99, e), this.x += o, this.collidesWithMap() && (this.x -= o, this.dx *= -Math.pow(.5, e), this.dy *= Math.pow(.8, e), this.dz *= Math.pow(.8, e)), this.y += d, this.collidesWithMap() && (this.y -= d, this.dx *= Math.pow(.8, e), this.dy *= -Math.pow(.5, e), this.dz *= Math.pow(.8, e)), this.z += s, this.collidesWithMap() && (this.z -= s, this.dx *= Math.pow(.8, e), this.dy *= Math.pow(.8, e), this.dz *= -Math.pow(.5, e)), this.ttl -= e, this.actor && this.actor.update()
    }
    ...
    if (this.ammo.rounds > 0) {
        if (this.player.isAtReady())
            if (this.actor) {
                if (0 == this.rofCountdown) {
                    this.shotsQueued++, this.rofCountdown = this.rof;
                    var e = new Comm.output(6);
                    e.packInt8(Comm.fire), e.packRad(this.player.pitch), e.packRadU(this.player.viewYaw), ws.send(e.buffer), 0 == this.automatic && (this.triggerPulled = !1)
                }
    ...
    if (this.ammo.rounds--, this.actor) this.actor.fire(), this.player.actor.fire(), this.player.id == meId && updateAmmoUi();
        ...
        var e = BABYLON.Matrix.RotationYawPitchRoll(this.player.viewYaw, this.player.pitch, 0),
            i = BABYLON.Matrix.Translation(0, 0, 1).multiply(e),
            t = .002 * (this.player.shotSpread + this.player.weapon.accuracy),
            r = BABYLON.Matrix.RotationYawPitchRoll((Math.random() - .5) * t, (Math.random() - .5) * t, (Math.random() - .5) * t),
            n = (i = i.multiply(r)).getTranslation(),
            a = BABYLON.Matrix.Translation(.1, .1, .4),
            o = (a = (a = a.multiply(e)).add(BABYLON.Matrix.Translation(this.player.x, this.player.y + .3, this.player.z))).getTranslation();
        this.fireMunitions(o, n)
    }
    this.player.shotSpread += this.shotSpreadIncrement, 0 == this.ammo.rounds && (this.actor && Sounds.hammerClick.play(), this.triggerPulled = !1)
}, Gun.prototype.startReload = function () {
    return 0 != this.ammo.store && 0 != this.player.isAtReady() && !this.triggerPulled && (this.ammo.rounds != this.ammo.capacity && (0 == this.ammo.rounds ? this.reloadCountdown = this.longReloadTime : this.reloadCountdown = this.shortReloadTime, this.actor && this.actor.reload(), !0))
}, Gun.prototype.reload = function () {
    var e = Math.min(Math.min(this.ammo.capacity, this.ammo.reload) - this.ammo.rounds, this.ammo.store);
    if (this.ammo.rounds += e, this.ammo.store -= e, this.actor) this.reloadCountdown = 0, this.player.id == meId && updateAmmoUi();
    else {
        var i = new Comm.output(1);
        i.packInt8(Comm.reload), send(i.buffer, this.player.id)
    }
    ...
    return 0 != this.player.isAtReady() && !this.triggerPulled && (this.player.stowWeaponCountdown = this.stowWeaponTime, this.actor && this.actor.stow(), !0)
}, Gun.prototype.isAtReady = function () {
    return 0 == this.shotsQueued && 0 == this.reloadCountdown && 0 == this.equipCountdown && 0 == this.player.stowWeaponCountdown
    ...
    Math.seed = t;
    for (var r = 0; r < 20; r++) {
        var n = Math.normalize3({
            x: i.x + Math.seededRandom(-.15, .15),
            y: i.y + Math.seededRandom(-.1, .1),
            z: i.z + Math.seededRandom(-.15, .15)
        });
        munitionsManager.fireBullet(this.player, e, n, this.damage, this.ttl, this.velocity)
    }
...
MunitionsManager.prototype.update = function (e) {
    for (var i = 0; i < 4; i++) this.bulletPool.forEachActive(function (i) {
        i.update(.25 * e), Object.keys(players).forEach(function (e) {
            var t = players[e];
            if (t && !t.isDead() && t.id != i.player.id && Math.abs(i.x - t.x) < .3 && Math.abs(i.y - (t.y + .3)) < .3 && Math.abs(i.z - t.z) < .3) {
                var r = i.player;
                v1.x = i.dx, v1.y = i.dy, v1.z = i.dz, v2.x = t.x - i.x, v2.y = t.y + .32 - i.y, v2.z = t.z - i.z;
                var n = BABYLON.Vector3.Cross(v1, v2);
                hitPlayer(t, r, i.damage / (1 + 20 * n.length()), v1.x, v1.z), i.remove()
            }
        })
    });
...
}, MunitionsManager.prototype.fireBullet = function (e, i, t, r, n, a) {
    this.bulletPool.retrieve().fire(e, i, t, r, n, a)
...
Player.prototype.update = function (e) {
    var i = 0,
        t = 0,
        r = 0;
    if (this.controlKeys & CONTROL.left && (i -= Math.cos(this.moveYaw), r += Math.sin(this.moveYaw)), this.controlKeys & CONTROL.right && (i += Math.cos(this.moveYaw), r -= Math.sin(this.moveYaw)), this.controlKeys & CONTROL.up && (this.climbing ? t += 1 : (i += Math.sin(this.moveYaw), r += Math.cos(this.moveYaw))), this.controlKeys & CONTROL.down && (this.climbing ? t -= 1 : (i -= Math.sin(this.moveYaw), r -= Math.cos(this.moveYaw))), this.climbing) {
        o = this.dy;
        this.dy += .014 * t * e;
        y = .5 * (this.dy + o) * e;
        this.y += y, this.dy *= Math.pow(.5, e), this.getOccupiedCell().cat != MAP.ladder && (this.y = Math.round(this.y), this.climbing = !1), this.collidesWithMap() && y > 0 && this.y % 1 > .3 && (this.y -= y, this.dy *= Math.pow(.5, e))
    } else {
        var n = new BABYLON.Vector3(i, t, r).normalize(),
            a = this.dx,
            o = this.dy,
            d = this.dz;
        this.dx += .007 * n.x * e, this.dz += .007 * n.z * e, this.dy -= .003 * e;
        var s = .5 * (this.dx + a) * e,
            y = .5 * (this.dy + o) * e,
            l = .5 * (this.dz + d) * e;
        if (this.x += s, this.collidesWithMap()) {
            x = this.y;
            this.y += Math.abs(s), this.collidesWithMap() && (this.x -= s, this.dx *= Math.pow(.5, e), this.y = x), this.lookForLadder()
        }
        if (this.z += l, this.collidesWithMap()) {
            var x = this.y;
            this.y += Math.abs(l), this.collidesWithMap() && (this.z -= l, this.dz *= Math.pow(.5, e), this.y = x), this.lookForLadder()
        }
        this.y += y, this.collidesWithMap() ? (y < 0 && (this.jumping = !1), this.y -= y, this.dy *= Math.pow(.5, e)) : 0 == this.jumping && (this.jumping = !0)
    }
    var z = Math.length3(this.dx, this.dy, this.dz);
    this.actor && this.id == meId && (z *= .75), this.climbing && (z *= 3), this.bobble = (this.bobble + 7 * z) % Math.PI2, this.shotSpread += Math.floor(150 * z), this.shotSpread *= this.weapon.accuracySettleFactor, this.dx *= Math.pow(.8, e), this.dz *= Math.pow(.8, e), this.weapon && this.weapon.update(e), this.hp > 0 && (this.hp = Math.min(100, this.hp + .05 * e)), !this.actor && this.stowWeaponCountdown > 0 && (this.stowWeaponCountdown -= e, this.stowWeaponCountdown <= 0 && (this.stowWeaponCountdown = 0, this.equipWeapon(this.equipWeaponIdx))), !this.actor || this.isDead() || this.actor.bodyMesh.isEnabled() || this.actor.respawn(this.x, this.y, this.z), this.actor || this.grenadeCountdown > 0 && (this.grenadeCountdown -= e, this.grenadeCountdown <= 0 && (this.grenadeThrown || (this.throwGrenade(), this.grenadeCountdown = 30, this.grenadeThrown = !0)))
}, Player.prototype.doAI = function () {
    Math.random() < .001 || playerTotal > botGoal && Math.random(), Math.random() < .01 && (this.viewYaw = 6.282 * Math.random() - 3.141)
}, Player.prototype.jump = function () {
    var e = !this.jumping | this.climbing;
    e || (this.y -= .2, this.collidesWithMap() && (e = !0), this.y += .2), e && (this.climbing ? (this.dy = .03, this.climbing = !1) : this.dy = .06, this.jumping = !0)
}, Player.prototype.equipWeapon = function (e) {
    if (this.weaponIdx = e, this.weapon = this.weapons[e], this.weapons[e].equip(), this.actor) this.id == meId && updateAmmoUi();
    else {
        var i = new Comm.output(3);
        i.packInt8(Comm.equipWeapon), i.packInt8(this.id), i.packInt8(this.equipWeaponIdx), sendToAll(i.buffer)
    }
}, Player.prototype.stowWeapon = function (e) {
    if (this.equipWeaponIdx = e, this.weapon.stow())
        if (this.actor)(i = new Comm.output(2)).packInt8(Comm.stowWeapon), i.packInt8(e), ws.send(i.buffer);
        else {
            var i = new Comm.output(2);
            i.packInt8(Comm.stowWeapon), i.packInt8(this.id), sendToOthers(i.buffer, this.id)
        }
    ...
    return this.weapon.isAtReady() && this.grenadeCountdown <= 0 && !this.isDead()
    ...
    this.isAtReady() && (this.grenadeThrown = !1, this.grenadeCountdown = 30, this.grenadeThrowPower = Math.clamp(e, 0, 1))
}, Player.prototype.throwGrenade = function () {
    if (this.grenadeCount > 0)
        if (this.actor) {
            if (this.grenadeCountdown > 0) {
                var e = new Comm.output(7);
                e.packInt8(Comm.throwGrenade), e.packRad(this.pitch), e.packRadU(this.viewYaw), e.packFloat(Math.clamp(grenadeThrowPower, 0, 1)), ws.send(e.buffer), grenadeThrowPower = -1e3
            }
        } else {
            this.grenadeCount--;
            var i = BABYLON.Matrix.RotationYawPitchRoll(this.viewYaw, this.pitch, 0),
                ...
                n = (r = (r = r.multiply(i)).add(BABYLON.Matrix.Translation(this.x, this.y + .3, this.z))).getTranslation(),
                a = .1 * this.grenadeThrowPower + .1;
            t.x *= a, t.y *= a, t.z *= a, (e = new Comm.output(14)).packInt8(Comm.throwGrenade), e.packInt8(this.id), e.packFloat(n.x), e.packFloat(n.y), e.packFloat(n.z), e.packFloat(t.x), e.packFloat(t.y), e.packFloat(t.z), sendToAll(e.buffer), munitionsManager.throwGrenade(this, n, t)
        }
}, Player.prototype.die = function () {
    this.hp = 0, this.killStreak = 0, this.controlKeys = 0, this.shotSpread = 0, this.jumping = !1, this.grenadeCountdown = 0, this.actor ? this.actor.die() : this.resetWeaponState()
}, Player.prototype.respawn = function (e, i, t) {
    this.x = e, this.y = i, this.z = t, this.hp = 100, this.actor && (this.actor.mesh.position.x = e, this.actor.mesh.position.y = i, this.actor.mesh.position.z = t, this.actor.respawn(), this.weapon.equip(), this.resetWeaponState(), this.id == meId && updateAmmoUi())
}, Player.prototype.resetWeaponState = function () {
    this.stowWeaponCountdown = 0;
    for (var e = 0; e < this.weapons.length; e++) this.weapons[e] && (this.weapons[e].ammo.rounds = this.weapons[e].ammo.capacity, this.weapons[e].ammo.store = this.weapons[e].ammo.storeMax, this.weapons[e].reloadCountdown = 0, this.weapons[e].rofCountdown = 0, this.weapons[e].shotsQueued = 0, this.weapons[e].triggerPulled = !1, this.weapons[e].equipCountdown = 0);
    ...
    for (var r, n, a, o, d, s, y, l, x, z = .25; z <= .75; z += .25)
        for (var h = 0; h <= .6; h += .3)
            for (var c = .25; c <= .75; c += .25)
                if (r = e + z, n = i + h, a = t + c, o = Math.floor(r), d = Math.floor(n), s = Math.floor(a), o >= 0 && d >= 0 && s >= 0 && o < map.width && d < map.height && s < map.depth) {
                    var u = map.data[o][d][s];
                    if (u.cat == MAP.ground || u.cat == MAP.block) return u;
                    if (u.cat == MAP.ramp) switch (y = r % 1, l = n % 1, x = a % 1, u.dir) {
                        case 0:
                            if (l < x) return u;
                            break;
                        case 2:
                        ...
                        if (y = r % 1, l = n % 1, x = a % 1, y < .7 && y > .3 && x < .7 && x > .3 && l < .5) return u
                    } else if (u.cat == MAP.column) {
                        if (y = r % 1 - .5, x = a % 1 - .5, y * y + x * x < .04) return u
                    } else if (u.cat == MAP.tank) {
                        if (y = r % 1 - .5, l = n % 1 - .5, x = a % 1 - .5, l < .2) return u;
                        if (0 == u.dir || 2 == u.dir) {
                            if (y * y + l * l < .25) return u
                        } else if (x * x + l * l < .25) return u
                    } else if (u.cat == MAP.lowWall && (y = r % 1, l = n % 1, x = a % 1, l < .25)) switch (u.dir) {
                        case 0:
                            if (x > .75) return u;
                            break;
                        case 1:
