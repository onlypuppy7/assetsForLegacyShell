    ...
    ws.onmessage = function (e) {
        for (var i = new Comm.input(e.data); i.isMoreDataAvailable();) switch (i.unPackInt8U()) {
            ...
            case Comm.addPlayer:
                ...
                NaN != t.hp && void 0 != t.hp || (console.log("Invisibility bug caught in Comm.addPlayer!"), t.hp = 100), players[t.id] || (meId == t.id || (t.name = t.name.replace(/<|>/g, ""), 0 == t.name.length ? t.name = "Anonymous" : (t.name = fixStringWidth(t.name), isBadWord(t.name) && (t.name = "!@#$"))), addPlayer(t));
                break;
            ...
            case Comm.sync:
                ...
                if (r == meId) {
                    k.serverX = a, k.serverY = n, k.serverZ = o;
            ...
            case Comm.ping:
                ...
                xe.style.color = ye < 100 ? "#0f0" : ye < 150 ? "#ff0" : ye < 200 ? "#f90" : "#f00", document.getElementById("PING").innerText = ye + "ms", timeout.set(function () {
                    var e = new Comm.output(1);
...
function loggedIn(e) {
    if (!gameSession && (dbId = e.uid, gameSession = null, e.getIdToken(!0).then(function (e) {
            ...
            t.onopen = function (i) {
                ...
                (dbStats = JSON.parse(e.data)).error ? (i.innerHTML = "Database currently undergoing maintenance. Customizations unavailable at this time. Sorry!", console.log("dbStats Error: " + JSON.stringify(dbStats.error))) : (updatePlayerStatsDisplay(), i.style.display = "none", document.getElementById("playerStats").style.display = "flex", gameSession = dbStats.session), t.close()
            }, t.onclose = function (e) {
                console.log("authWs closed: " + e.code + " " + e.reason)
            }, t.onerror = function (e) {
                i.innerHTML = "Database currently undergoing maintenance. Customizations unavailable at this time. Sorry!", console.log("authWS Error: " + JSON.stringify(e, ["message", "arguments", "type", "name"]))
            }
        }).catch(function (e) {
            console.log(e)
...
function play(e) {
    if (console.log("play(" + e + ")"), settings.volume > 0 && BABYLON.Engine.audioEngine.audioContext.resume && BABYLON.Engine.audioEngine.audioContext.resume(), 0 != (username = document.getElementById("username").value.trim()).length) {
...
function joinGame() {
    ...
    document.getElementById("alert").focus(), openAlertDialog("CONNECTING", "Please wait!", null, null, !0), console.log("Connecting to: " + e), (ws = new WebSocket(e)).binaryType = "arraybuffer", ws.onopen = function (e) {
        ...
        freezeFrame || (e.code == CloseCode.gameNotFound ? (console.log("Game not found - id: " + uniqueId + ", key: " + uniqueKey), openAlertDialog("GAME NOT FOUND", "Sorry! This game ID is either<br>invalid, or no longer exists.", {
            label: "OK"
        }), uniqueId = 0, uniqueKey = 0, document.getElementById("joinCode").value = "") : e.code == CloseCode.gameFull ? (console.log("Game full - id: " + uniqueId + ", key: " + uniqueKey), openAlertDialog("GAME FULL", "Sorry, this game is currently full!<br>Wait a moment, or try another one!", {
            label: "OK"
        })) : e.code == CloseCode.badName ? (closeAlertDialog(), openAlertDialog("INVALID NAME", "I'm going to guess you know why.", {
            label: "Yes"
        }), (e = document.getElementById("username")).value = "", e.disabled = !1, e.focus(), document.getElementById("playButton").disabled = !1) : e.code == CloseCode.mainMenu ? console.log("WebSocket closing - returning to Main Menu") : inGame ? (console.log("Connection lost: " + e.code + " " + e.reason), openAlertDialog("CONNECTION LOST", "Please try a different server,<br> or try again later!", {
            label: "OK",
            onclick: reloadPage
        })) : (console.log("Cannot connect: " + e.code + " " + e.reason), openAlertDialog("CANNOT CONNECT", "Please try a different server, reload<br>the page, or try again later!<br><br><small>(code: " + e.code + ")</small><br>", {
            label: "OK",
            onclick: closeAlertDialog
        })))
    }, ws.onmessage = function (e) {
...
function menuIn(e, i) {
    ...
    var r = 0,
        d = interval.set(function () {
            r += .2, t.style.opacity = r, t.style.transform = "scale(" + (.2 * r + .8) + ")", t.style.transform += " rotate(" + (15 * r - 15) + "deg)", r >= 1 && (interval.clear(d), i && i())
        }, 33)
}
...
function menuOut(e) {
    var i = document.getElementById(e),
        t = 1,
        r = interval.set(function () {
            t -= .2, i.style.opacity = t, i.style.transform = "scale(" + (.2 * t + .8) + ")", i.style.transform += " rotate(" + (15 - 15 * t) + "deg)", t <= 0 && (i.style.display = "none", interval.clear(r))
        }, 33)
}
...
function loadSounds(e, i) {
    ...
    }, Sounds.hammerClick = new BABYLON.Sound("", "sound/hammerClick.mp3", e), Sounds.ammo = new BABYLON.Sound("", "sound/ammo.mp3", e), Sounds.shellBurst = new BABYLON.Sound("", "sound/shellBurst.mp3", e, t, r), Sounds.hit = new BABYLON.Sound("", "sound/hit.mp3", e, t, r), Sounds.grenade = {
    ...
    for (var a = 1; a < 11; a++) Sounds.death.push(new BABYLON.Sound("", "sound/death/scream" + a + ".mp3", e, t, r))
}
...
! function (e, i, t, r, d, a, n) {
    ...
    openAlertDialog("SETTING UP", "Wait for it...", null, null, !0), chatInEl = document.getElementById("chatIn"), chatOutEl = document.getElementById("chatOut"), killEl = document.getElementById("killTicker"), aipBanner = document.getElementById("aipBanner"), resize(), document.exitPointerLock = document.exitPointerLock || document.mozExitPointerLock || document.webkitExitPointerLock;
    var e = "",
        i = 0;
...
PlayerActor.prototype.applyStamp = function (e) {
    ...
    if (this.player.id != meId) {
        ...
        if (p != this.bodyMesh.isVisible && (this.mesh.setVisible(p), p ? this.showNameSprite() : this.hideNameSprite()), this.nameSprite) {
            var g = this.mesh.position.clone();
            g.y += .5 * this.nameSprite.height + .65;
            var f = new BABYLON.Vector3(camera.globalPosition.x - g.x, camera.globalPosition.y - g.y, camera.globalPosition.z - g.z).normalize();
            f.scaleInPlace(.4), f.addInPlace(g), this.nameSprite.position = f
        }
    }
...
Player.prototype.update = function (e, i) {
    ...
    if (this.controlKeys & CONTROL.left && (t -= Math.cos(this.moveYaw), d += Math.sin(this.moveYaw)), this.controlKeys & CONTROL.right && (t += Math.cos(this.moveYaw), d -= Math.sin(this.moveYaw)), this.controlKeys & CONTROL.up && (this.climbing ? r += 1 : (t += Math.sin(this.moveYaw), d += Math.cos(this.moveYaw))), this.controlKeys & CONTROL.down && (this.climbing ? r -= 1 : (t -= Math.sin(this.moveYaw), d -= Math.cos(this.moveYaw))), this.climbing) {
        ...
        var n = this.dx + this.clientCorrection.x,
            o = this.dy,
            s = this.dz + this.clientCorrection.y;
        this.clientCorrection.x = 0, this.clientCorrection.y = 0, this.dx += .007 * a.x * e, this.dz += .007 * a.z * e, this.dy -= .003 * e, this.dy = Math.max(-.2, this.dy);
        var y = .5 * (this.dx + n) * e,
            x = .5 * (this.dy + o) * e,
            l = .5 * (this.dz + s) * e;
