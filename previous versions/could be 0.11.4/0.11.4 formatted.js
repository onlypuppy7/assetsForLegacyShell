...
function onCanvasClick() {
    if (this.requestPointerLock(), me && me.isDead() && respawnTime < 0) {
...
function setUpSocket() {
    ws.onmessage = function (e) {
        for (var i, t = new Comm.input(e.data); i = t.unPackInt8U();) switch (i) {
            ...
            case Comm.addPlayer:
                ...
                players[r.id] || (meId == r.id ? addChat("Welcome, " + r.name + "!") : 0 == r.name.length ? r.name = "Anonymous" : (r.name = fixNameWidth(r.name), isBadWord(r.name) && (r.name = "!@#$")), addPlayer(r));
                break;
            case Comm.removePlayer:
                removePlayer(y = t.unPackInt8U());
                break;
            case Comm.spawnItem:
                var a = t.unPackInt16U(),
                    n = t.unPackInt8U(),
                    o = t.unPackFloat(),
                    d = t.unPackFloat(),
                    s = t.unPackFloat();
                itemManager.spawnItem(a, n, o, d, s);
                break;
            case Comm.collectItem:
                var y = t.unPackInt8U(),
                    ...
                    l = t.unPackInt8U(),
                    a = t.unPackInt16U();
                itemManager.collectItem(n, a), y == meId && (me.collectItem(n, l), updateAmmoUi());
                break;
            case Comm.keyDown:
                var y = t.unPackInt8U(),
                    h = t.unPackInt8U();
                (Z = players[y]) && (Z.controlKeys |= h);
                break;
            case Comm.keyUp:
                var y = t.unPackInt8U(),
                    h = t.unPackInt8U();
            ...
            case Comm.jump:
                y = t.unPackInt8U();
                (Z = players[y]) && players[y].jump();
                break;
            case Comm.die:
                var x, z = t.unPackInt8U(),
                    c = t.unPackInt8U(),
                    u = t.unPackInt8U(),
                    m = players[z],
                    p = players[c];
                p ? (x = p.name, p.isDead() || (p.totalKills++, p.killStreak++, p.bestKillStreak = Math.max(p.bestKillStreak, p.killStreak), c == meId && (kills++, bestKillStreak = Math.max(bestKillStreak, p.killStreak)))) : x = "N/A";
                var g;
                if (m) {
                    g = m.name;
                    var f = new BABYLON.Vector3(m.x, m.y + .32, m.z);
                    m.actor.deathSound.setPosition(f), m.actor.deathSound.play();
                    var w = Math.randomInt(0, Sounds.death.length);
                    Sounds.death[w].setPosition(f), Sounds.death[w].play(), m.actor.explodeMesh.setEnabled(!0), m.actor.whiteMesh.setEnabled(!0), m.actor.yolkMesh.setEnabled(!0), beginAnimation(m.actor.explodeMesh, 0, 50, !1, 1), scene.beginAnimation(m.actor.whiteMesh, 0, 50, !1, 1), scene.beginAnimation(m.actor.yolkMesh, 0, 56, !1, 1), shellFragBurst(m.actor.mesh, 200, 1), m.die(), m.resetStateBuffer(), m.actor.mesh.position.x = m.x, m.actor.mesh.position.y = m.y, m.actor.mesh.position.z = m.z
                } else g = "N/A";
                if (z != meId) {
                    if (c == meId) {
                        var v = document.getElementById("KILL_BOX");
                        v.style.display = "block", document.getElementById("KILLED_NAME").innerText = g;
                        var M = document.getElementById("KILL_STREAK");
                        me.killStreak > 1 ? M.innerText = me.killStreak + "-KILL STREAK" : M.innerText = "";
                        var b = 1.5,
                            S = setInterval(function () {
                                v.style.transform = "scale(" + b + "," + b + ")", (b -= .05) <= 1 && (b = 1, clearInterval(S))
                            }, 33);
                    ...
                    deaths++, camera.parent = null, camera.position = new BABYLON.Vector3(me.actor.mesh.position.x, me.actor.mesh.position.y + .2, me.actor.mesh.position.z), p && (camera.lockedTarget = p.actor.bodyMesh), document.exitPointerLock(), respawnTime = u;
                    var B = document.getElementById("DEATH_BOX");
                    B.style.display = "block", document.getElementById("KILLED_BY_NAME").innerText = x;
                    var b = 2,
                        S = setInterval(function () {
                            B.style.transform = "scale(" + b + "," + b + ")", (b -= .05) <= 1 && (b = 1, clearInterval(S))
                        }, 33),
                        k = document.getElementById("RESPAWN");
                    k.innerText = "";
                    var I = setInterval(function () {
                        k.innerText = "You may respawn in " + respawnTime, --respawnTime < 0 && (clearInterval(I), k.innerText = "CLICK TO RESPAWN!")
                    }, 1e3)
                }
                rebuildPlayerList(), updateBestStreakUi();
                break;
            case Comm.chat:
                addChat(t.unPackString());
                break;
            case Comm.sync:
                var a = t.unPackInt8U(),
                    C = t.unPackInt8U(),
                    ...
                    E = t.unPackRad(),
                    P = t.unPackInt8U();
                if (!(Z = players[a])) break;
                if (a == meId) {
                    if (Z.serverX = o, Z.serverY = d, Z.serverZ = s, me.jumping) return;
                    var L = me.stateIdx - C;
                    L < 0 && (L = stateBufferSize - C + me.stateIdx);
                    X = Math.mod(C + Math.floor(L / 2) - 1, stateBufferSize);
                    var N = Z.previousStates[X],
                        O = Z.x,
                        T = (Z.y, Z.z);
                    me.x = o, me.y = d, me.z = s, me.dx = N.dx, me.dy = N.dy, me.dz = N.dz, me.climbing = P, me.jumping = N.jumping;
                    for (var F = me.controlKeys; X != me.stateIdx; X = Math.mod(X + 1, stateBufferSize)) N = me.previousStates[X], me.controlKeys = N.controlKeys, me.moveYaw = N.moveYaw, N.jump && me.jump(), me.update(N.delta, !0), me.previousStates[X].x = me.x, me.previousStates[X].y = me.y, me.previousStates[X].z = me.z, me.previousStates[X].dx = me.dx, me.previousStates[X].dy = me.dy, me.previousStates[X].dz = me.dz;
                    if (Math.length2(me.x - O, me.z - T) < .5) {
                        var Y = Math.length2(me.dx, me.dz),
                            R = Math.max(4 - 64 * Y, 1);
                        me.x = O + (me.x - O) / R, me.z = T + (me.z - T) / R
                    }
                    me.controlKeys = F
                } else Z.x = o, Z.y = d, Z.z = s, Z.viewYaw = A, Z.moveYaw = A, Z.pitch = E, Z.climbing = P;
                break;
            case Comm.fireBullet:
                var a = t.unPackInt8U(),
                    ...
                    U = t.unPackFloat();
                if (!(Z = players[a])) break;
                a != meId && (Z.actor.head.rotation.x = Z.pitch, Z.actor.mesh.rotation.y = Z.viewYaw, Z.weapon.actor.fire()), munitionsManager.fireBullet(Z, {
                    x: o,
                    y: d,
                    z: s
                }, {
                    ...
                    y: G,
                    z: U
                }, Z.weapon.damage, Z.weapon.ttl, Z.weapon.velocity);
                break;
            case Comm.fireShot:
                var a = t.unPackInt8U(),
                    o = t.unPackFloat(),
                    ...
                    W = t.unPackInt8U();
                if (!(Z = players[a])) break;
                a != meId && (Z.actor.head.rotation.x = Z.pitch, Z.actor.mesh.rotation.y = Z.viewYaw, Z.weapon.actor.fire()), Math.seed = W;
                for (X = 0; X < 20; X++) {
                    var H = Math.normalize3({
                        x: D + Math.seededRandom(-.15, .15),
                        y: G + Math.seededRandom(-.1, .1),
                        z: U + Math.seededRandom(-.15, .15)
                    });
                    munitionsManager.fireBullet(Z, {
                        x: o,
                        y: d,
                        z: s
                    }, H, Z.weapon.damage, Z.weapon.ttl, Z.weapon.velocity * Math.seededRandom(.9, 1.1))
                }
                break;
            case Comm.throwGrenade:
                var a = t.unPackInt8U(),
                    ...
                    V = t.unPackFloat();
                if (!(Z = players[a])) break;
                Z.grenadeCount--, a != meId ? (Z.actor.head.rotation.x = Z.pitch, Z.actor.mesh.rotation.y = Z.viewYaw) : updateAmmoUi(), Z.actor.throwGrenade(), munitionsManager.throwGrenade(Z, {
                    x: o,
                    y: d,
                    z: s
                }, {
            ...
            case Comm.reload:
                a = t.unPackInt8U();
                players[a] && players[a].reload();
                break;
            case Comm.swapWeapon:
                var a = t.unPackInt8U(),
                    q = t.unPackInt8U();
                players[a] && players[a].swapWeapon(q);
                break;
            case Comm.hitMe:
                var _ = t.unPackInt8U(),
                    K = t.unPackFloat(),
                    V = t.unPackFloat();
                me.hp = _, me.actor.hit(), hitIndicator.hit(K, V);
                break;
            case Comm.hitThem:
                var a = t.unPackInt8U(),
                    _ = t.unPackInt8U();
                if (!(Z = players[a])) break;
                Z.hp = _, Z.actor.hit(), _ > 0 && shellFragBurst(Z.actor.mesh, 100, 1);
                break;
            case Comm.respawn:
                var a = t.unPackInt8U(),
                    o = t.unPackFloat(),
                    d = t.unPackFloat(),
                    s = t.unPackFloat();
                (Z = players[a]) && (Z.respawn(o, d, s), a == meId && (document.getElementById("DEATH_BOX").style.display = "none", camera.position = BABYLON.Vector3.Zero(), camera.rotation = BABYLON.Vector3.Zero(), camera.rotationQuaternion = BABYLON.Quaternion.Zero(), camera.parent = me.actor.eye, camera.lockedTarget = null));
                break;
            case Comm.switchTeam:
                ...
                var Z = players[a];
                if (Z) {
                    Z.team = Q, a == meId && (myTeam = Q);
                    for (var X = 0; X < 20; X++)(Z = players[X]) && Z.actor && Z.actor.updateTeam();
                    rebuildPlayerList()
                }
                break;
            case Comm.ping:
                var J = Date.now() - pingStartTime;
                pingTotal += J, pingSamples++;
                var $ = document.getElementById("PING");
                $.style.color = J < 100 ? "#0f0" : J < 150 ? "#ff0" : J < 200 ? "#f90" : "#f00", document.getElementById("PING").innerText = J + "ms", setTimeout(function () {
                    var e = new Comm.output(1);
...
function update() {
    ...
    if (fpsSamples++, fpsTotal += i, fpsSamples % 10 == 0) {
        document.getElementById("FPS").innerHTML = "FPS: " + i, document.getElementById("healthBar").style.width = me.hp + "%";
        var t = Date.now() - pingStartTime;
        highestPing = Math.max(highestPing, t)
    }
    for (var r = Date.now(), a = r - lastTimeStamp, n = Math.min(a, 17); lastTimeStamp < r - .01;) {
        lastTimeStamp += n;
        a = n / 17;
        if (freezeFrame) me.update(a), me.actor.update(a);
        else
            for (var o = 0; o < players.length; o++) {
                var d = players[o];
                d && (d.update(a), d.actor && d.actor.update(a))
            }
        me && (me.stateIdx = Math.mod(me.stateIdx + 1, stateBufferSize), me.previousStates[me.stateIdx] = {
            delta: a,
            moveYaw: me.moveYaw,
            fire: !1,
            jump: !1,
        ...
        }, me.stateIdx % e == 0 && ws.readyState == ws.OPEN && me.hp > 0 && !freezeFrame && serverSync(), hitIndicator.update(a), reticle.update(a), me.weapon && 0 == me.weapon.ammo.rounds && (me.stateIdx % 20 == 0 ? document.getElementById("AMMO").style.color = "#f00" : me.stateIdx % 20 == 10 && (document.getElementById("AMMO").style.color = "#fff")), 1 == grenadePowerUp && (grenadeThrowPower = Math.min(grenadeThrowPower + .015, 1)) > 0 && (document.getElementById("grenadeThrowContainer").style.visibility = "visible", document.getElementById("grenadeThrow").style.height = 100 * grenadeThrowPower + "%")), freezeFrame || (munitionsManager.update(a), itemManager.update(a), updateExplosions(explosionSmokeManager, a), updateExplosions(explosionFireManager, a))
    }
}
...
function serverSync() {
    if (me) {
        me.moveYaw = me.viewYaw, me.previousStates[me.stateIdx].moveYaw = me.moveYaw;
...
function addChat(e) {
    var i = document.getElementById("chatOut");
...
function onKeyDown(e) {
    if (!keyIsDown[e.keyCode]) {
        keyIsDown[e.keyCode] = !0;
        var i = e.key.toLocaleUpperCase();
        if ("" != inputTally && "" == (inputTally = inputTally.replace(i, "")) && (document.getElementById("help").style.display = "none", localStorage.setItem("hideHelp", 1)), " " == i && (i = "SPACE", e.preventDefault()), debug) {
            ...
            if ("\\" == i) {
                freezeFrame = !0;
                for (t = 0; t < scene.particleSystems.length; t++) scene.particlesPaused = !0;
                for (var t = 0; t < 20; t++) players[t] && players[t].actor && (players[t].actor.mesh.setVisible(!0), players[t].actor.showNameSprite());
                ws.close()
            }
        }
        inputDown(i)
...
function Scope() {
    var e = new BABYLON.SpriteManager("", "img/scope.png?v=1", 1, 256, scene);
    e.fogEnabled = !1, e.layerMask = 536870912, this.sprite = new BABYLON.Sprite("", e), this.crosshairs = new BABYLON.AbstractMesh("", scene), this.crosshairs.setEnabled(!1), this.crosshairs.position.z = 1;
    var i = [new BABYLON.Vector3(-1, 0, 0), new BABYLON.Vector3(1, 0, 0)],
        t = BABYLON.MeshBuilder.CreateLines("", {
            points: i
        }, scene);
    t.layerMask = 536870912, t.color = BABYLON.Color3.Black(), t.parent = this.crosshairs, i = [new BABYLON.Vector3(0, -1, 0), new BABYLON.Vector3(0, 1, 0)], (t = BABYLON.MeshBuilder.CreateLines("", {
        points: i
...
function Reticle() {
    this.mesh = new BABYLON.AbstractMesh("reticle", scene), this.lines = [], Meshes.reticle.setMaterial(Materials.ui);
    for (var e = 0; e < 4; e++) {
...
function isMeshVisible(e, i) {
    for (var t = i || e.getBoundingInfo().boundingBox.center.z, r = e.position.x - camera.globalPosition.x, a = e.position.y + t - camera.globalPosition.y, n = e.position.z - camera.globalPosition.z, o = Math.length3(r, a, n), d = Math.normalize3({
        ...
        var z = collidesWithCell(s += d.x, y += d.y, l += d.z);
        if (z && (z.cel.cat == MAP.ground || z.cel.cat == MAP.block) && 2 == ++h) return !1
    }
    return !0
}
...
function onResourcesLoaded() {
    ...
    window.selectClass = function (i) {
        selectedClass = Math.mod(selectedClass + i, a), localStorage.setItem("selectedClass", selectedClass), z -= i, e()
...
function sendBugReport() {
    var e = "Version: " + version;
    if (e += "\nFacebook ID: " + facebookId, e += "\nPointer lock: " + (document.pointerLockElement ? "true" : "false"), e += "\n\nGame session: " + gameSession + "\nPing: " + Math.floor(pingTotal / pingSamples) + "\nFPS: " + Math.ceil(fpsSum / 60) + "\nGame type: " + gameType + "\nPrivate game: " + privateGame + "\nKills: " + kills + "\nDeaths: " + deaths + "\nBest kill streak: " + bestKillStreak + "\nRespawn time: " + respawnTime + "\nHighest ping: " + highestPing, me) {
        var i = {};
        for (var t in me) i[t] = me[t];
        delete i.weapon, delete i.weapons, delete i.actor, delete i.previousStates, delete i.ws, e += "\n\nPlayer: " + objToStr(i), stateLog && (e += "\n\n" + objToStr(stateLog), stateLog = null);
        for (var t in me.weapons) {
            var r = {};
            for (var a in me.weapons[t]) r[a] = me.weapons[t][a];
            delete r.player, delete r.actor, e += "\n\nWeapon: " + objToStr(r)
        }
    }
    ...
    var n = document.getElementById("bugEmail"),
        o = document.getElementById("bugDescription"),
        d = {
            name: "Bug Report",
            email: n.value || "not@provided.com",
            comments: o.value + "\n\n" + lastErrorMessage + "\n\n" + e
        },
        s = [];
    ...
    }, closeBugReport(), n.value = "", o.value = "", h.open("POST", location.protocol + "//" + location.hostname + "/feedback.php", !0), h.setRequestHeader("Content-type", "application/x-www-form-urlencoded"), h.send(l)
}
...
function refactorConfigKeys(e) {
    document.getElementsByName("config").forEach(function (i) {
...
function configKey(e) {
    ...
    e = e || window.event, i == controlEl ? (1 == e.button && event.preventDefault(), i.style.fontWeight = "bold", i.style.color = "#fff", i.innerText = "MOUSE " + e.button, setControl(i)) : (controlEl && (controlEl.style.fontWeight = "bold", controlEl.style.color = "#fff", controlEl.innerText = controlEl.oldText), i.oldText = i.innerText, i.style.fontWeight = "normal", i.style.color = "#edc", i.innerText = "Press key or button", controlEl = i, i.focus(), inGame && releaseKeys(), window.onkeydown = function (e) {
        if (27 != (e.which || e.keyCode)) {
            var t = e.key;
            return " " == t && (t = "space", e.preventDefault()), i.style.fontWeight = "bold", i.style.color = "#fff", i.innerText = t.toLocaleUpperCase(), setControl(i), e.stopPropagation(), !1
        }
...
function showMainMenu() {
    if (openAlertDialog("LOADING", "Just a moment!", null, null, !0), document.getElementById("homeButton").style.visibility = "hidden", document.getElementById("friendsButton").style.visibility = "hidden", document.getElementById("game").style.display = "none", document.getElementById("inviteFriends").style.display = "none", resetGameUI(), inGame && (canvas.removeEventListener("click", onCanvasClick), canvas.removeEventListener("mousedown", onCanvasMouseDown), canvas.removeEventListener("mouseup", onCanvasMouseUp), canvas.removeEventListener("mousemove", onCanvasMouseMove), ws.close(CloseCode.mainMenu), ws = null, releaseKeys(), gameStartTime > 0)) {
...
function collidesWithCell(e, i, t) {
    if (e < 0 || e >= map.width || t < 0 || t >= map.depth || i < 0 || i >= map.height) return !1;
    var r = Math.floor(e),
        a = Math.floor(i),
        n = Math.floor(t),
        o = map.data[r][a][n];
...
function Player(e) {
    this.id = e.id, this.name = e.name, this.charClass = e.charClass, this.team = e.team, this.shellColor = e.shellColor, this.x = e.x, this.y = e.y, this.z = e.z, this.dx = e.dx, this.dy = e.dy, this.dz = e.dz, this.viewYaw = e.viewYaw, this.controlKeys = e.controlKeys, this.moveYaw = e.moveYaw, this.pitch = e.pitch, this.totalKills = e.totalKills, this.totalDeaths = e.totalDeaths, this.killStreak = e.killStreak, this.bestKillStreak = e.bestKillStreak, this.hp = e.hp, this.weaponIdx = e.weaponIdx, this.serverX = e.x, this.serverY = e.y, this.serverZ = e.z, this.clientCorrection = {
        x: 0,
        y: 0
    }, this.aimTarget = {
        x: 0,
        y: 0,
        z: 0
    }, this.rofCountdown = 0, this.triggerPulled = !1, this.shotsQueued = 0, this.reloadsQueued = 0, this.recoilCountdown = 0, this.reloadCountdown = 0, this.swapWeaponCountdown = 0, this.weaponSwapsQueued = 0, this.equipWeaponIdx = this.weaponIdx, this.shotSpread = 0, this.grenadeCount = 1, this.grenadeCapacity = 3, this.grenadeCountdown = 0, this.grenadesQueued = 0, this.jumping = !1, this.climbing = !1, this.bobble = 0, this.stateIdx = 0, this.timeOfDeath = 0, this.ready = !1, this.teamSwitchCooldown = 0, this.previousStates = [];
    ...
    void 0 !== PlayerActor && (this.actor = new PlayerActor(this)), this.weapons = [new Cluck9mm(this), new classes[this.charClass].weapon(this)], this.weapon = this.weapons[this.weaponIdx], this.actor && this.weapon.actor.equip()
}
...
function isBadWord(e) {
    var i = (e = " " + e + " ").toLowerCase().replace(/[^a-zA-Z0-9|!\|@|$| ]/g, "").replace(/6|g/g, "9").replace(/b/g, "6").replace(/\||l|i|1/g, "!").replace(/e/g, "3").replace(/a|@/g, "4").replace(/o/g, "0").replace(/s|\$/g, "5").replace(/t/g, "7").replace(/z/g, "2").replace(/7h3|my|y0ur|7h3!r|h!5|h3r/g, ""),
        t = i.search(/( 94y | cum| 455 | 6u77 )/);
    i.replace(/ /g, "");
    var r = /(r374r|4u7!5|d0wn55|B00B|8d|p0rn|5w4!!0w|347d!ck|347m3|347my|d!k|0r4!|5p0093|fuk|j!2|5u!c!d|m4573r6|5p0063|5p3rm|p3nu5|pu55y|6u7753x|fux|6u77h0!3|4n4!|4nu5|k!!!b!4ck5|murd3rb!4ck5|h!7!3r|w3764ck|49!n4|94y|455h0!3|5uck|j3w|5p!c|ch!nk|n!994|n!993r|n!663r|5h!7|6!7ch|fuck|cun7|kkk|wh0r3|f49|7w47|p3n!|r4p3w0m|r4p39!r|r4p!57|r4p3r|r4p!n|c0ck|7!75|900k|d!ckh34d)/,
        a = i.search(r);
    i.replace(/(.)(?=\1)/g, "");
    var n = i.search(r);
    return t > -1 || a > -1 || n > -1
...
BulletActor.prototype.fire = function () {
    ...
    this.mesh.position.x = this.bullet.x, this.mesh.position.y = this.bullet.y, this.mesh.position.z = this.bullet.z, this.mesh.scaling.z = Math.min(this.mesh.scaling.z + .05, 3)
...
var nameTexture, nameSprites, bulletHoleManager, explosionSmokeManager, explosionFireManager, respawnTime, players, keyIsDown, map, inputTally, light, camera, uiCamera, me, grenadeThrowPower, grenadePowerUp = !1,
    ...
    killDisplayTimeout, SPS, rotInc = Math.PI / 2,
...
Scope.prototype.show = function () {
    ...
    this.sprite.size = e + 20, this.crosshairs.scaling.x = e / 2, this.crosshairs.scaling.y = e / 2, this.crosshairs.setEnabled(!0), camera.viewport.width = e / engine.getRenderWidth(), camera.viewport.x = .5 - .5 * camera.viewport.width
}, Scope.prototype.hide = function () {
    this.sprite.size = 0, this.crosshairs.setEnabled(!1), camera.viewport.width = 1, camera.viewport.x = 0
...
! function (e, i, t, r, a, n, o) {
    ...
    function e(e) {
        console.log(e), facebookId = null, gameSession = null, x = e;
        var i = document.getElementById("ritekit-alerts");
        i && (i.style.visibility = "hidden"), "connected" === e.status ? (document.getElementById("fbLoginRequest").style.display = "none", document.getElementById("statsRetrieval").style.display = "block", facebookId = e.authResponse.userID, getRequest("!host_specific/requestUserData.php?id=" + e.authResponse.userID + "&token=" + e.authResponse.accessToken, function (e) {
            try {
                var i = JSON.parse(e);
    ...
    if (resize(), BABYLON.Engine.isSupported()) {
        selectedServer = getStoredNumber("selectedServer", 0);
        for (var i = document.getElementById("serverSelect"), t = 0; t < servers.length; t++)(a = servers[t]).el = document.createElement("option"), a.el.textContent = a.name, i.appendChild(a.el);
        i.selectedIndex = selectedServer;
        for (var r = 1e6, t = 0; t < servers.length; t++) {
            var a = servers[t];
            (s = a.el).pinger = new WebSocket("wss://" + servers[t].address), s.pinger.pingStart = Date.now(), s.pinger.idx = t, s.pinger.onopen = function (e) {
                var t = Date.now() - e.target.pingStart;
                t < r && (selectServer(e.target.idx), i.selectedIndex = selectedServer, r = t), e.target.close()
            }
        }
        settings.volume = getStoredNumber("volume", 1), settings.mouseSensitivity = Math.max(Math.min(11, getStoredNumber("mouseSensitivity", 5)), 0), settings.mouseInvert = getStoredNumber("mouseInvert", 1), settings.holdToAim = getStoredBool("holdToAim", !0), settings.autoDetail = getStoredBool("autoDetail", !0), settings.shadowsEnabled = getStoredBool("shadowsEnabled", !0), settings.highRes = getStoredBool("highRes", !0), gameType = getStoredNumber("gameType", GameType.ffa), document.getElementById("volume").value = settings.volume, document.getElementById("mouseSensitivity").value = settings.mouseSensitivity, document.getElementById("mouseInvert").checked = -1 == settings.mouseInvert, document.getElementById("holdToAim").checked = settings.holdToAim, document.getElementById("autoDetail").checked = settings.autoDetail, document.getElementById("shadowsEnabled").checked = settings.shadowsEnabled, document.getElementById("highRes").checked = settings.highRes, setDetailSettingsVisibility(settings.autoDetail);
        var n = JSON.parse(localStorage.getItem("controlConfig"));
        for (var o in n) n.hasOwnProperty(o) && (inputToControlMap[o] = n[o]);
        for (var d in inputToControlMap) inputToControlMap.hasOwnProperty(d) && d != d.toLocaleUpperCase() && (inputToControlMap[d.toLocaleUpperCase()] = inputToControlMap[d], delete inputToControlMap[d]);
        for (var o in inputToControlMap) {
            var s = document.getElementById(inputToControlMap[o]);
            s && (s.innerText = o.toLocaleUpperCase(), s.style.fontWeight = "bold", s.style.color = "#fff")
        }
        for (var y = document.getElementById("playerList"), l = document.getElementById("playerSlot"), t = 0; t < 20; t++) {
            var h = l.cloneNode(!0);
            y.appendChild(h)
        }
        ...
        if (window.checkLoginState = function () {
            ...
            mapTest.x = Number(z[1]), mapTest.y = Number(z[2]), mapTest.z = Number(z[3]), mapTest.pitch = Number(z[4]), mapTest.yaw = Number(z[5])
        }
        ...
        c.value = getStoredString("lastUsername", ""), c.addEventListener("keyup", function (e) {
            c.value = fixNameWidth(e.target.value), "Enter" != e.code && 13 != e.keyCode || c.value.length > 0 && (c.disabled = !0, login())
        }), playOffline ? (ws = {
            send: function () {}
        }, inGame = !0, startGame()) : showMainMenu()
    } else openAlertDialog("OH, NO!", 'Shell Shockers requires WebGL, but your<br>browser is not configured to support it.<br><br><a style="text-decoration: underline" target="_window" href="https://shellshock.io/faq.html#webgl">Click here to learn more</a>')
...
PlayerActor.prototype.drawTextOnNameTexture = function (e, i, t, r, a, n) {
    ...
    if (x = this.player.jumping ? 0 : Math.length3(this.player.dx, this.player.dy, this.player.dz), this.bobbleIntensity += (x - this.bobbleIntensity) / 10, this.bodyMesh.rotation.z = 5 * i, this.bodyMesh.position.y = 1.5 * t + .32, shake > 0)
        if ((shake *= .9) < .001) shake = 0;
            ...
            var z = Math.random() * shake - .5 * shake,
                c = Math.random() * shake - .5 * shake,
                u = Math.random() * shake - .5 * shake;
            this.eye.rotation.x += (z - this.eye.rotation.x) / 10, this.eye.rotation.y += (c - this.eye.rotation.y) / 10, this.eye.rotation.z += (u - this.eye.rotation.z) / 10
        }
    ...
    GameMap = {
        ...
        generateMap: function (e, i, t, r) {
            ...
            for (s = 0; s < a.width; s++) {
                ...
                for (z = 0; z < a.height; z++) a.data[s][z] = Array(a.depth).fill({})
            }
            for (c = 0; c < a.width * a.depth * a.height * .2;)
                for (var n = Math.seededRandomInt(4, 8), o = Math.seededRandomInt(4, 8), d = Math.seededRandomInt(2, c % a.height), s = Math.seededRandomInt(1, a.width - 1 - n), y = Math.seededRandomInt(1, a.depth - 1 - o), l = (Math.seededRandomInt(1, 4), s); l < s + n; l++)
                    for (p = y; p < y + o; p++)
                        for (var h = 0; h < d; h++) {
                            var x = l == s || p == y || l == s + n - 1 || p == y + o - 1;
                            a.data[l][h][p].cat || c++, a.data[l][h][p] = x ? {
                                cat: 1,
                                dec: 4,
            ...
            for (s = 0; s < a.width; s++)
                for (z = 0; z < a.height; z++)
                    for (y = 0; y < a.depth; y++) 0 == a.data[s][z][y].dec && (a.data[s][z][y] = {});
            for (u = 0; u < a.width * a.depth * a.height / 2; u++) {
                var s = Math.seededRandomInt(0, a.width),
                    z = 2 * Math.seededRandomInt(0, Math.floor(a.height / 2)),
                    y = Math.seededRandomInt(0, a.depth);
                1 == a.data[s][z][y].cat && a.data[s][z][y].dec > 0 && 4 == GameMap.numNeighbors6(a, s, z, y) && (a.data[s][z][y] = {})
            }
            for (var c = 0; c < a.width * a.depth * a.height / 60;) {
                var s = Math.seededRandomInt(1, a.width - 1),
                    z = Math.seededRandomInt(0, a.height - 1),
                    y = Math.seededRandomInt(1, a.depth - 1);
                a.data[s][z][y].cat || 0 != z && 1 != a.data[s][z - 1][y].cat || (1 != a.data[s][z][y + 1].cat || a.data[s][z + 1][y + 1].cat || a.data[s][z][y - 1].cat || 0 != z && 1 != a.data[s][z - 1][y - 1].cat ? 1 != a.data[s + 1][z][y].cat || a.data[s + 1][z + 1][y].cat || a.data[s - 1][z][y].cat || 0 != z && 1 != a.data[s - 1][z - 1][y].cat ? 1 != a.data[s][z][y - 1].cat || a.data[s][z + 1][y - 1].cat || a.data[s][z][y + 1].cat || 0 != z && 1 != a.data[s][z - 1][y + 1].cat ? 1 != a.data[s - 1][z][y].cat || a.data[s - 1][z + 1][y].cat || a.data[s + 1][z][y].cat || 0 != z && 1 != a.data[s + 1][z - 1][y].cat || (a.data[s][z][y] = {
                    cat: 2,
                    dec: 0,
                    dir: 3
                }, a.data[s][z + 1][y] = {}, a.data[s + 1][z + 1][y] = {}, c++) : (a.data[s][z][y] = {
                    cat: 2,
                    dec: 0,
                    dir: 2
                }, a.data[s][z + 1][y] = {}, a.data[s][z + 1][y + 1] = {}, c++) : (a.data[s][z][y] = {
                    cat: 2,
                    dec: 0,
                    dir: 1
                }, a.data[s][z + 1][y] = {}, a.data[s - 1][z + 1][y] = {}, c++) : (a.data[s][z][y] = {
                    ...
                    dec: 0,
                    dir: 0
                }, a.data[s][z + 1][y] = {}, a.data[s][z + 1][y - 1] = {}, c++))
            }
            for (u = 0; u < a.width * a.depth * a.height / 10; u++) {
                var s = Math.seededRandomInt(1, a.width - 1),
                    z = Math.seededRandomInt(0, a.height - 1),
                    y = Math.seededRandomInt(1, a.depth - 1);
                !a.data[s][z][y].cat && (0 == z || 1 == a.data[s][z - 1][y].cat) && GameMap.numNeighbors26(a, s, z, y) < 11 && (a.data[s][z][y] = {
                    cat: 4,
                    dec: 0,
                    dir: 0
                })
            ...
            for (var u = 0; u < a.width * a.depth * a.height / 10; u++) {
                var s = Math.seededRandomInt(1, a.width - 1),
                    z = Math.seededRandomInt(0, a.height - 1),
                    y = Math.seededRandomInt(1, a.depth - 1);
                if (!a.data[s][z][y].cat && (0 == z || 1 == a.data[s][z - 1][y].cat && 4 == a.data[s][z - 1][y].dec) && GameMap.numNeighbors26(a, s, z, y) < 11) {
                    a.data[s][z][y] = {
                        cat: 1,
                    ...
                    for (var m = 0; m < 20; m++) {
                        ...
                        if (4 == a.data[p][z][p].cat) break;
                        if (!a.data[p][z][p].cat && (0 == z || 1 == a.data[l][z - 1][p].cat && 4 == a.data[l][z - 1][p].dec)) {
                            a.data[l][z][p] = {
                                cat: 4,
                                dec: 0,
                                dir: 0
            ...
            for (s = 0; s < a.width; s++)
                for (y = 0; y < a.depth; y++)
                    for (z = 0; z < a.height - 1; z++) !a.data[s][z][y].cat && this.numNeighbors6(a, s, z, y) >= 4 && !a.data[s][z + 1][y].cat && (a.data[s][z][y] = {
                        cat: 1,
                        dec: this.firstNeighborDec(a, s, z, y),
                        dir: Math.seededRandomInt(0, 4)
                    });
            for (s = 0; s < a.width; s++)
                for (z = 0; z < a.height; z++)
                    for (y = 0; y < a.depth; y++) a.data[s][z][y].cat && 6 == GameMap.numNeighbors6(a, s, z, y) && (a.data[s][z][y].cat = 1, a.data[s][z][y].dec = 0);
...
Grenade.prototype.update = function (e) {
        ...
        if (munitionsManager.grenadePool.recycle(this), this.actor) {
            ...
            for (var t = this, r = 0; r < 20; r++) {
                ...
                if (a && (a.id == t.player.id || 0 == a.team || a.team != t.player.team)) {
                    ...
                    if (a && !a.isDead() && y > 0) {
                        for (var l = Math.normalize3({
                                ...
                                y: o,
                                z: d
                            }, .25), h = a.x, x = a.y + .3, z = a.z, c = !0, u = 0; u < s; u += .25) {
                            if (collidesWithCell(h, x, z)) {
                                c = !1;
                                break
                            }
                            h += l.x, x += l.y, z += l.z
                        }
                        c && hitPlayer(a, t.player, y, -n, -d)
                    }
                }
            } else {
                var m = this.dx,
                ...
                var f = .5 * (this.dx + m) * e,
                    w = .5 * (this.dy + p) * e,
                    v = .5 * (this.dz + g) * e;
                this.dx *= Math.pow(.99, e), this.dz *= Math.pow(.99, e), this.x += f, this.collidesWithMap() && (this.x -= f, this.dx *= -Math.pow(.5, e), this.dy *= Math.pow(.8, e), this.dz *= Math.pow(.8, e)), this.y += w, this.collidesWithMap() && (this.y -= w, this.dx *= Math.pow(.8, e), this.dy *= -Math.pow(.5, e), this.dz *= Math.pow(.8, e)), this.z += v, this.collidesWithMap() && (this.z -= v, this.dx *= Math.pow(.8, e), this.dy *= Math.pow(.8, e), this.dz *= -Math.pow(.5, e)), this.ttl -= e, this.actor && this.actor.update()
            }
    ...
    return this.actor ? (this.ammo.store += this.ammo.pickup, !0) : this.ammo.store < this.ammo.storeMax && (this.ammo.store = Math.min(this.ammo.storeMax, this.ammo.store + this.ammo.pickup), !0)
...
Player.prototype.update = function (e, i) {
    ...
    if (this.controlKeys & CONTROL.left && (t -= Math.cos(this.moveYaw), a += Math.sin(this.moveYaw)), this.controlKeys & CONTROL.right && (t += Math.cos(this.moveYaw), a -= Math.sin(this.moveYaw)), this.controlKeys & CONTROL.up && (this.climbing ? r += 1 : (t += Math.sin(this.moveYaw), a += Math.cos(this.moveYaw))), this.controlKeys & CONTROL.down && (this.climbing ? r -= 1 : (t -= Math.sin(this.moveYaw), a -= Math.cos(this.moveYaw))), this.climbing) {
        ...
        this.dy += .014 * r * e;
        l = .5 * (this.dy + d) * e;
        this.y += l, this.dy *= Math.pow(.5, e), this.getOccupiedCell().cat != MAP.ladder && (this.y = Math.round(this.y), this.climbing = !1), this.collidesWithMap() && l > 0 && this.y % 1 > .3 && (this.y -= l, this.dy *= Math.pow(.5, e))
    } else {
    ...
    this.actor && this.id == meId && (x *= .75), this.climbing && (x *= 3), this.dx *= Math.pow(.8, e), this.dz *= Math.pow(.8, e), i || (this.bobble = (this.bobble + 7 * x) % Math.PI2, this.shotSpread += Math.floor(150 * x), this.shotSpread *= Math.pow(this.weapon.accuracySettleFactor, e), this.weapon && this.weapon.update(e), this.hp > 0 && (this.hp = Math.min(100, this.hp + .05 * e)), this.swapWeaponCountdown > 0 && (this.shotSpread = this.weapon.shotSpreadIncrement, this.swapWeaponCountdown -= e, !this.actor && this.swapWeaponCountdown <= 0 && (this.swapWeaponCountdown = 0, this.weaponIdx = this.equipWeaponIdx, this.weapon = this.weapons[this.weaponIdx])), this.reloadCountdown > 0 && (this.shotSpread = this.weapon.shotSpreadIncrement, this.reloadCountdown -= e, this.reloadCountdown <= 0 && (this.reloadCountdown = 0, this.reloaded())), this.rofCountdown > 0 && (this.rofCountdown = Math.max(this.rofCountdown - e, 0)), this.recoilCountdown > 0 && (this.recoilCountdown = Math.max(this.recoilCountdown - e, 0)), this.teamSwitchCooldown > 0 && (this.teamSwitchCooldown = Math.max(this.teamSwitchCooldown - e, 0)), this.grenadeCountdown > 0 && (this.grenadeCountdown -= e, this.grenadeCountdown <= 0 && this.grenadesQueued > 0 && !this.actor && this.throwGrenade()), this.triggerPulled && this.fire()), this.actor || (this.shotsQueued > 0 && this.fire(), this.reloadsQueued > 0 && this.reload(), this.weaponSwapsQueued > 0 && this.swapWeapon(this.equipWeaponIdx))
    ...
    if (this.x += e, this.collidesWithMap()) {
        var t = this.y;
        this.y += Math.abs(e) + .01 * i, this.collidesWithMap() && (this.x -= e, this.dx *= Math.pow(.5, i), this.y = t), this.lookForLadder()
    }
}, Player.prototype.moveZ = function (e, i) {
    if (this.z += e, this.collidesWithMap()) {
        var t = this.y;
        this.y += Math.abs(e) + .01 * i, this.collidesWithMap() && (this.z -= e, this.dz *= Math.pow(.5, i), this.y = t), this.lookForLadder()
    }
    ...
    if (this.actor && this.id != meId) this.weapon.actor.reload();
        ...
        if (this.actor) this.weapon.actor.reload(), this.triggerPulled = !1, (e = new Comm.output(1)).packInt8(Comm.reload), ws.send(e.buffer);
        else {
            var e = new Comm.output(2);
            e.packInt8(Comm.reload), e.packInt8(this.id), sendToOthers(e.buffer, this.id), this.reloadsQueued--
        }
    ...
    this.weapon.ammo.rounds += e, this.weapon.ammo.store -= e, this.actor && this.id == meId && updateAmmoUi()
    ...
    this.hp = 0, this.killStreak = 0, this.controlKeys = 0, this.shotSpread = 0, this.jumping = !1, this.climbing = !1, this.resetWeaponState(), this.actor && (this.actor.die(), grenadePowerUp = !1)
}, Player.prototype.respawn = function (e, i, t) {
    this.x = e, this.y = i, this.z = t, this.hp = 100, this.resetStateBuffer(), this.resetWeaponState(), this.actor && (this.actor.mesh.position.x = e, this.actor.mesh.position.y = i, this.actor.mesh.position.z = t, this.actor.respawn(), this.weapon.equip(), this.id == meId && updateAmmoUi())
}, Player.prototype.resetWeaponState = function () {
    this.rofCountdown = 0, this.triggerPulled = !1, this.shotsQueued = 0, this.reloadsQueued = 0, this.recoilCountdown = 0, this.reloadCountdown = 0, this.swapWeaponCountdown = 0, this.weaponSwapsQueued = 0, this.shotSpread = 0, this.weaponIdx = 1, this.grenadeCountdown = 0, this.grenadesQueued = 0;
    for (var e = 0; e < this.weapons.length; e++) this.weapons[e] && (this.weapons[e].ammo.rounds = this.weapons[e].ammo.capacity, this.weapons[e].ammo.store = this.weapons[e].ammo.storeMax);
    ...
    if (this.controlKeys & CONTROL.up) {
        ...
        if (e.cat == MAP.ladder) {
            ...
            if (Math.abs(Math.radDifference(Math.cardToRad(e.dir), this.moveYaw)) < Math.PI90 / 3) switch (e.dir) {
                case 0:
                    i > .3 && i < .7 && t > .5 && (this.z = Math.floor(this.z) + .74, this.climbing = !0);
                    break;
                case 1:
                    t > .3 && t < .7 && i > .5 && (this.x = Math.floor(this.x) + .74, this.climbing = !0);
                    break;
                case 2:
                    i > .3 && i < .7 && t < .5 && (this.z = Math.floor(this.z) + .26, this.climbing = !0);
                    break;
                case 3:
                    t > .3 && t < .7 && i < .5 && (this.x = Math.floor(this.x) + .26, this.climbing = !0)
            }
        }
    }
    ...
    for (var r, a, n, o, d, s, y, l, h, x = .25; x <= .75; x += .25)
        for (var z = 0; z <= .6; z += .3)
            for (var c = .25; c <= .75; c += .25)
                if (r = e + x, a = i + z, n = t + c, o = Math.floor(r), d = Math.floor(a), s = Math.floor(n), o >= 0 && d >= 0 && s >= 0 && o < map.width && d < map.height && s < map.depth) {
